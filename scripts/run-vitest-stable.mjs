...
const nonServerProjects = [
  "@mercuryai/shared",
  "@mercuryai/db",
  "@mercuryai/adapter-utils",
  "@mercuryai/adapter-codex-local",
  "@mercuryai/adapter-opencode-local",
  "@mercuryai/ui",
  "mercuryai",
];
...
function runVitest(args, label) {
  console.log(`\n[test:run] ${label}`);
  invocationIndex += 1;
  const testRoot = mkdtempSync(path.join(os.tmpdir(), `mercury-vitest-${process.pid}-${invocationIndex}-`));
  const env = {
    ...process.env,
    MERCURY_HOME: path.join(testRoot, "home"),
    MERCURY_INSTANCE_ID: `vitest-${process.pid}-${invocationIndex}`,
    TMPDIR: path.join(testRoot, "tmp"),
  };
  mkdirSync(env.MERCURY_HOME, { recursive: true });
...
runVitest(
  ["--project", "@mercuryai/server", ...excludeRouteArgs],
