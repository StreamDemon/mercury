...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Mercury data dir to use
  MERCURY_HOST=127.0.0.1    Host passed to the onboarded server
  MERCURY_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/mercury-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/mercury-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export MERCURY_HOST="${MERCURY_HOST:-127.0.0.1}"
export MERCURY_PORT="${MERCURY_PORT:-3100}"
export MERCURY_OPEN_ON_LISTEN="${MERCURY_OPEN_ON_LISTEN:-false}"
...
echo "MERCURY_HOST: $MERCURY_HOST"
echo "MERCURY_PORT: $MERCURY_PORT"
...
env \
  ...
  HOST="$MERCURY_HOST" \
  PORT="$MERCURY_PORT" \
  MERCURY_OPEN_ON_LISTEN="$MERCURY_OPEN_ON_LISTEN" \
  pnpm mercuryai onboard --yes --data-dir "$PC_DATA"
