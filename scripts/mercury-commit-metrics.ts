...
const DEFAULT_QUERY = "\"Co-Authored-By: Mercury <noreply@modernmethod.io>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/mercury-commit-metrics-cache.json");
...
const MERCURY_EMAIL = "noreply@modernmethod.io";
const MERCURY_NAME = "mercury";
...
function printHelp() {
  console.log(`Usage: tsx scripts/mercury-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === MERCURY_EMAIL) || (displayName && displayName.toLowerCase() === MERCURY_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Mercury commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "mercury-commit-metrics",
