#!/usr/bin/env node
/**
 * Screenshot utility for Mercury UI.
 *
 * Reads the board token from ~/.mercury/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/mercury-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".mercury/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.mercury/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Mercury origin only
