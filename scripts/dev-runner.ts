...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[mercury] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`mercuryai worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".mercury", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-mercury-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".mercury/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[mercury] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[mercury] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[mercury] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  MERCURY_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.MERCURY_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.MERCURY_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.MERCURY_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.MERCURY_DEV_SERVER_STATUS_TOKEN;
  env.MERCURY_MIGRATION_PROMPT ??= "never";
  env.MERCURY_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[mercury] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.MERCURY_BIND = effectiveBind;
  if (bindHost) {
    env.MERCURY_BIND_HOST = bindHost;
  } else {
    delete env.MERCURY_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.MERCURY_DEPLOYMENT_MODE;
    delete env.MERCURY_DEPLOYMENT_EXPOSURE;
    delete env.MERCURY_AUTH_BASE_URL_MODE;
    console.log("[mercury] dev mode: local_trusted (bind=loopback)");
  } else {
    env.MERCURY_DEPLOYMENT_MODE = "authenticated";
    env.MERCURY_DEPLOYMENT_EXPOSURE = "private";
    env.MERCURY_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[mercury] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.MERCURY_BIND;
  delete env.MERCURY_BIND_HOST;
  delete env.MERCURY_DEPLOYMENT_MODE;
  delete env.MERCURY_DEPLOYMENT_EXPOSURE;
  delete env.MERCURY_AUTH_BASE_URL_MODE;
  console.log("[mercury] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[mercury] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@mercuryai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[mercury] Command failed with code ${status.code}: pnpm --filter @mercuryai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[mercury] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.MERCURY_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[mercury] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[mercury] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@mercuryai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[mercury] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@mercuryai/server", serverScript, ...forwardedArgs],
