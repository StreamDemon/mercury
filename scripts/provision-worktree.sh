...
base_cwd="${MERCURY_WORKSPACE_BASE_CWD:?MERCURY_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${MERCURY_WORKSPACE_CWD:?MERCURY_WORKSPACE_CWD is required}"
mercury_home="${MERCURY_HOME:-$HOME/.mercury}"
mercury_instance_id="${MERCURY_INSTANCE_ID:-default}"
mercury_dir="$worktree_cwd/.mercury"
worktree_config_path="$mercury_dir/config.json"
worktree_env_path="$mercury_dir/.env"
worktree_name="${MERCURY_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${MERCURY_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.mercury/config.json" || -L "$base_cwd/.mercury/config.json" ) ]]; then
  source_config_path="$base_cwd/.mercury/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$mercury_home/instances/$mercury_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm mercuryai --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm mercuryai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v mercuryai >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      mercuryai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  MERCURY_WORKTREES_DIR="${MERCURY_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const mercuryDir = process.env.MERCURY_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.MERCURY_WORKTREES_DIR) ?? "~/.mercury-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(mercuryDir, "config.json");
  const envPath = path.resolve(mercuryDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "mercury",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.MERCURY_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.MERCURY_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.MERCURY_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "MERCURY_HOME=" + JSON.stringify(worktreeHome),
    "MERCURY_INSTANCE_ID=" + JSON.stringify(instanceId),
    "MERCURY_CONFIG=" + JSON.stringify(configPath),
    "MERCURY_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "MERCURY_IN_WORKTREE=true",
    "MERCURY_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.MERCURY_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("MERCURY_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated Mercury worktree config at $worktree_config_path" >&2
else
  if mercuryai_command_available; then
    run_isolated_worktree_init
  else
    echo "mercuryai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".mercury-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
