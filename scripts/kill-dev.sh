#!/usr/bin/env bash
#
# Kill all local Mercury dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all mercury dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/mercury(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.mercury/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.mercury/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.mercury/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/mercury*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.mercury/instances/*/db/postmaster.pid
    "$sibling_root"/.mercury/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No Mercury dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Mercury dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Mercury node processes..."
