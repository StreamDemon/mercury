...
COMPANY_ID="${COMPANY_ID:-${MERCURY_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-mercury-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-mercury-openclaw-smoke}"
...
Provide one of:
  MERCURY_AUTH_HEADER=\"Bearer <board-token>\"
  MERCURY_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Mercury OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.mercury.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
