...
MERCURY_HOST_PORT="${MERCURY_HOST_PORT:-3100}"
MERCURY_HOST_FROM_CONTAINER="${MERCURY_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.mercury-openclaw.override.yml"
...
detect_mercury_base_url() {
  ...
  for candidate in "$MERCURY_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${MERCURY_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${MERCURY_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
mercury_base_url="$(detect_mercury_base_url || true)"
...
Mercury URL for OpenClaw container:
EOF
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${MERCURY_HOST_PORT}
  (Do not use http://127.0.0.1:${MERCURY_HOST_PORT} inside the container.)
  If Mercury rejects the host, run on host machine:
    pnpm mercuryai allowed-hostname host.docker.internal
  Then restart Mercury and re-run this script.
EOF
  fi
  cat <<EOF
