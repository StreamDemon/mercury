...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.MERCURY_SECRETS_PROVIDER === undefined) {
    process.env.MERCURY_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.MERCURY_SECRETS_STRICT_MODE === undefined) {
    process.env.MERCURY_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.MERCURY_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.MERCURY_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.MERCURY_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.MERCURY_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set MERCURY_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@mercury.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.MERCURY_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://mercury:mercury@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "mercury");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://mercury:mercury@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "mercury");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: mercury");
    }

    const embeddedConnectionString = `postgres://mercury:mercury@127.0.0.1:${port}/mercury`;
  ...
  const configuredApiUrl = process.env.MERCURY_API_URL?.trim() || runtimeApiUrl;
  process.env.MERCURY_LISTEN_HOST = runtimeListenHost;
  process.env.MERCURY_LISTEN_PORT = String(listenPort);
  process.env.MERCURY_RUNTIME_API_URL = runtimeApiUrl;
  process.env.MERCURY_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.MERCURY_API_URL = configuredApiUrl;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.MERCURY_OPEN_ON_LISTEN === "true") {
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Mercury server failed to start");
    process.exit(1);
  });
}
