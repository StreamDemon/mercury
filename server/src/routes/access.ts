...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "mercury" &&
    normalized !== "mercury-create-agent" &&
    normalized !== "mercury-create-plugin" &&
    normalized !== "mercury-converting-plans-to-tasks" &&
...
/** Resolve the Mercury repo skills directory (built-in / managed skills). */
function resolveMercurySkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const mercurySkillsDir = resolveMercurySkillsDir();

  // Build set of Mercury-managed skill names
  const mercurySkillNames = new Set<string>();
  if (mercurySkillsDir) {
    try {
      for (const entry of fs.readdirSync(mercurySkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) mercurySkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isMercuryManaged: mercurySkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.mercuryApiUrl)) {
    const legacyMercuryApiUrl = nonEmptyTrimmedString(input.mercuryApiUrl);
    if (legacyMercuryApiUrl) merged.mercuryApiUrl = legacyMercuryApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    mercuryApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.mercuryApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawMercuryApiUrl =
    typeof defaults.mercuryApiUrl === "string"
      ? defaults.mercuryApiUrl.trim()
      : "";
  if (rawMercuryApiUrl) {
    try {
      const parsedMercuryApiUrl = new URL(rawMercuryApiUrl);
      if (
        parsedMercuryApiUrl.protocol !== "http:" &&
        parsedMercuryApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_mercury_api_url_protocol",
          level: "warn",
          message: `mercuryApiUrl must use http:// or https:// (got ${parsedMercuryApiUrl.protocol}).`
        });
      } else {
        normalized.mercuryApiUrl = parsedMercuryApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_mercury_api_url_configured",
          level: "info",
          message: `mercuryApiUrl set to ${parsedMercuryApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_mercury_api_url_invalid",
        level: "warn",
        message: `Invalid mercuryApiUrl: ${rawMercuryApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Mercury host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Mercury is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm mercuryai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/mercury`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/mercury-claimed-api-key.json and load MERCURY_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: mercuryApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Mercury hostname is reachable and allowed via `pnpm mercuryai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Mercury API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/mercury/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Mercury OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          mercuryApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Mercury generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "mercuryApiUrl": "https://mercury-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Mercury before key claim is allowed.
    ...
    - ~/.openclaw/workspace/mercury-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/mercury-claimed-api-key.json

    And set the MERCURY_API_KEY and MERCURY_API_URL in your environment variables as specified here:
    ...
    Then set MERCURY_API_KEY and MERCURY_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Mercury skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your MERCURY_API_URL to the top of your skill and note the path to your MERCURY_API_URL
    ...
    ${
      ...
      "Ensure Mercury is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Mercury base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.mercuryApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm mercuryai allowed-hostname <host>
      - then restart Mercury and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@mercury.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "mercury", path: "/api/skills/mercury" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "mercury-create-agent",
          path: "/api/skills/mercury-create-agent"
        },
        {
          name: "mercury-converting-plans-to-tasks",
          path: "/api/skills/mercury-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "mercury",
          path: `/api/invites/${token}/skills/mercury`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "mercury") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              mercuryApiUrl: req.body.mercuryApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.mercuryApiUrl &&
          !persistedDefaults.mercuryApiUrl
        ) {
          missingPersistedFields.push("mercuryApiUrl");
        }
