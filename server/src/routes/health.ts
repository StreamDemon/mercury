...
import { heartbeatRuns, instanceUserRoles, invites } from "@mercuryai/db";
...
function hasDevServerStatusToken(providedToken: string | undefined) {
  const expectedToken = process.env.MERCURY_DEV_SERVER_STATUS_TOKEN?.trim();
...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    const exposeDevServerDetails =
      exposeFullDetails || hasDevServerStatusToken(req.get("x-mercury-dev-server-status-token"));
