...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@mercuryai/db";
...
import { trackAgentCreated } from "@mercuryai/shared/telemetry";
...
import { runClaudeLogin } from "@mercuryai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@mercuryai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@mercuryai/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@mercuryai/adapter-gemini-local";
...
export function agentRoutes(
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeMercurySkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readMercurySkillSyncPreference(
