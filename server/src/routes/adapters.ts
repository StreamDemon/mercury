 ...
 * toggle server-side adapter code for the whole Mercury instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-mercury-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@mercuryai/adapter-hermes@0.2.0" → packageName + version
  ...
  // without modifying Mercury's source code.
  //
