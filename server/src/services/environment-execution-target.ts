...
import type { Environment, EnvironmentLease } from "@mercuryai/shared";
import {
...
export async function resolveEnvironmentExecutionTarget(input: {
  ...
  if (input.environment.driver === "sandbox") {
    ...
    const mercuryApiUrl =
      typeof input.leaseMetadata?.mercuryApiUrl === "string" && input.leaseMetadata.mercuryApiUrl.trim().length > 0
        ? input.leaseMetadata.mercuryApiUrl.trim()
        : typeof process.env.MERCURY_RUNTIME_API_URL === "string" && process.env.MERCURY_RUNTIME_API_URL.trim().length > 0
          ? process.env.MERCURY_RUNTIME_API_URL.trim()
          : null;
