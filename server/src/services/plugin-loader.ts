 ...
 *    (`~/.mercury/plugins/`) and `node_modules` for packages matching
 *    the `mercury-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Mercury plugins.
 * Packages matching this pattern are considered Mercury plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "mercury-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: MercuryPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.mercury/plugins/ local directory
  | "npm"               // npm packages matching mercury-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: MercuryPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the mercury-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "mercury-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: MercuryPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Mercury plugins installed as npm packages in the current
   * Node.js environment matching the "mercury-plugin-*" naming convention.
   *
   ...
   * the "mercuryPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Mercury plugin.
   * Throws if the package is a Mercury plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<MercuryPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: MercuryPluginManifestV1;
    newManifest: MercuryPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Mercury plugin naming convention.
 * Accepts both the "mercury-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @mercuryai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "mercuryPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const mercuryPlugin = pkgJson["mercuryPlugin"];
  if (
    mercuryPlugin !== null &&
    typeof mercuryPlugin === "object" &&
    !Array.isArray(mercuryPlugin)
  ) {
    const manifestRelPath = (mercuryPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: MercuryPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumMercuryVersion;
}
 ...
 *   packageName: "mercury-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: MercuryPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as MercuryPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Mercury plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<MercuryPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Mercury plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasMercuryPlugin = "mercuryPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasMercuryPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<MercuryPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: MercuryPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @mercuryai/shared exports). Run those workers through
