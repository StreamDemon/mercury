...
import { claudeConfigDir, parseClaudeStreamJson } from "@mercuryai/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@mercuryai/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@mercuryai/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveMercuryInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "mercury-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "mercury-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "mercury-feedback-v1";
const FEEDBACK_DESTINATION = "mercury_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "mercury_run" &&
    file.source !== "mercury_run_events" &&
    file.source !== "mercury_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.MERCURY_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readMercurySkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let mercuryRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      mercuryRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "mercury/run.json",
        contentType: "application/json",
        source: "mercury_run",
        contents: `${JSON.stringify(mercuryRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.mercuryRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "mercury/run-events.json",
        contentType: "application/json",
        source: "mercury_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "mercury/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "mercury_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.mercuryRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
