...
export function pluginDatabaseService(db: Db) {
  ...
  async function ensureNamespace(pluginId: string, manifest: MercuryPluginManifestV1) {
  ...
  return {
    ...
    async applyMigrations(pluginId: string, manifest: MercuryPluginManifestV1, packageRoot: string) {
      if (!manifest.database) return null;
