...
import {
  ...
  writeMercurySkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".mercury.yaml" || normalized === ".mercury.yml") return "extension";
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .mercury.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .mercury.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .mercury.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .mercury.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Mercury will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .mercury.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .mercury.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[mercuryExtensionPath];
  if (selectedFiles.has(mercuryExtensionPath) && typeof extensionEntry === "string") {
    filtered[mercuryExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findMercuryExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".mercury.yaml"] === "string") return ".mercury.yaml";
  if (typeof files[".mercury.yml"] === "string") return ".mercury.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.mercury.yaml") || entry.endsWith("/.mercury.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "mercury_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/StreamDemon/mercury/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "mercury_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.mercurySkillKey = skill.key;
  metadata.mercury = {
    ...(isPlainRecord(metadata.mercury) ? metadata.mercury : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const mercuryExtensionPath = findMercuryExtensionPath(normalizedFiles);
  const mercuryExtension = mercuryExtensionPath
    ? parseYamlFrontmatter(readPortableTextFile(normalizedFiles, mercuryExtensionPath) ?? "")
    : {};
  const mercuryCompany = isPlainRecord(mercuryExtension.company) ? mercuryExtension.company : {};
  const mercurySidebar = normalizePortableSidebarOrder(mercuryExtension.sidebar);
  const mercuryAgents = isPlainRecord(mercuryExtension.agents) ? mercuryExtension.agents : {};
  const mercuryProjects = isPlainRecord(mercuryExtension.projects) ? mercuryExtension.projects : {};
  const mercuryTasks = isPlainRecord(mercuryExtension.tasks) ? mercuryExtension.tasks : {};
  const mercuryRoutines = isPlainRecord(mercuryExtension.routines) ? mercuryExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(mercuryCompany.brandColor),
      logoPath: asString(mercuryCompany.logoPath) ?? asString(mercuryCompany.logo),
      attachmentMaxBytes:
        typeof mercuryCompany.attachmentMaxBytes === "number" && Number.isFinite(mercuryCompany.attachmentMaxBytes)
          ? Math.max(1, Math.floor(mercuryCompany.attachmentMaxBytes))
          : null,
      requireBoardApprovalForNewAgents:
        typeof mercuryCompany.requireBoardApprovalForNewAgents === "boolean"
          ? mercuryCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof mercuryCompany.feedbackDataSharingEnabled === "boolean"
          ? mercuryCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof mercuryCompany.feedbackDataSharingConsentAt === "string"
          ? mercuryCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(mercuryCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(mercuryCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: mercurySidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(mercuryAgents[slug]) ? mercuryAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(mercuryProjects[slug]) ? mercuryProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(mercuryTasks[slug]) ? mercuryTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(mercuryRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(mercuryRoutines[slug]) ? mercuryRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  const strictSecretsMode = process.env.MERCURY_SECRETS_STRICT_MODE === "true";
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeMercurySkillSyncPreference({ ...adapterConfig }, desiredSkills);
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".mercury.yaml" ||
          relative === ".mercury.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const mercuryAgentsOut: Record<string, Record<string, unknown>> = {};
    const mercuryProjectsOut: Record<string, Record<string, unknown>> = {};
    const mercuryTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const mercuryRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readMercurySkillSyncPreference(
        ...
        mercuryAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      mercuryProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      mercuryTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      mercuryRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const mercuryExtensionPath = ".mercury.yaml";
    const mercuryAgents = Object.fromEntries(
      Object.entries(mercuryAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const mercuryProjects = Object.fromEntries(
      Object.entries(mercuryProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const mercuryTasks = Object.fromEntries(
      Object.entries(mercuryTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const mercuryRoutines = Object.fromEntries(
      Object.entries(mercuryRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[mercuryExtensionPath] = buildYamlFile(
      {
        schema: "mercury/v1",
        ...
        agents: Object.keys(mercuryAgents).length > 0 ? mercuryAgents : undefined,
        projects: Object.keys(mercuryProjects).length > 0 ? mercuryProjects : undefined,
        tasks: Object.keys(mercuryTasks).length > 0 ? mercuryTasks : undefined,
        routines: Object.keys(mercuryRoutines).length > 0 ? mercuryRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, mercuryExtensionPath);
