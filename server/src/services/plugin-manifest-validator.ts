 ...
 */
import { pluginManifestV1Schema } from "@mercuryai/shared";
import type { MercuryPluginManifestV1 } from "@mercuryai/shared";
import { PLUGIN_API_VERSION } from "@mercuryai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): MercuryPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as MercuryPluginManifestV1,
        };
      }
