 ...
 * ~/.mercury/adapter-plugins.json. This is the source of truth for which
...
import { resolveMercuryHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-mercury-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const mercuryDir = resolveMercuryHomeDir();
  return {
    adapterPluginsDir: path.join(mercuryDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(mercuryDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(mercuryDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "mercury-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Mercury external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
