...
import { trackAgentFirstHeartbeat } from "@mercuryai/shared/telemetry";
...
import { extractSkillMentionIds } from "@mercuryai/shared";
...
const DEFERRED_WAKE_CONTEXT_KEY = "_mercuryWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const MERCURY_WAKE_PAYLOAD_KEY = "mercuryWake";
const MERCURY_HARNESS_CHECKOUT_KEY = "mercuryHarnessCheckedOut";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const REPO_ONLY_CWD_SENTINEL = "/__mercury_repo_only__";
...
export function applyRunScopedMentionedSkillKeys(
  ...
  const existingPreference = readMercurySkillSyncPreference(config);
  return writeMercurySkillSyncPreference(config, [
...
export function compactRunLogChunk(chunk: string, maxChars = MAX_PERSISTED_LOG_CHUNK_CHARS) {
  ...
  const marker = `\n[mercury truncated run log chunk: omitted ${omittedChars} chars]\n`;
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[MERCURY_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[MERCURY_WAKE_PAYLOAD_KEY];
  }
  return merged;
}

async function buildMercuryWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  continuationSummary?:
  ...
  return {
    ...
    livenessContinuation: readNonEmptyString(input.contextSnapshot.livenessContinuationState) ||
      ...
      : null,
    checkedOutByHarness: input.contextSnapshot[MERCURY_HARNESS_CHECKOUT_KEY] === true,
...
export function buildMercuryTaskMarkdown(input: {
  issue: {
    id: string;
    identifier: string | null;
    title: string;
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  async function cancelQueuedRunForBlockedDependencies(
    ...
    const now = new Date();
    const reason =
      "Cancelled because issue dependencies are still blocked; Mercury will wake the assignee when blockers resolve";
  ...
  async function executeRun(runId: string) {
    ...
    if (continuationSummary) {
      context.mercuryContinuationSummary = {
        key: continuationSummary.key,
        title: continuationSummary.title,
      ...
      delete context.mercuryContinuationSummary;
    }
    const mercuryWakePayload = await buildMercuryWakePayload({
      db,
      companyId: agent.companyId,
      contextSnapshot: context,
      continuationSummary,
    ...
    if (mercuryWakePayload) {
      context[MERCURY_WAKE_PAYLOAD_KEY] = mercuryWakePayload;
    } else {
      delete context[MERCURY_WAKE_PAYLOAD_KEY];
    }
    const taskMarkdown = buildMercuryTaskMarkdown({
      issue: issueRef
        ? {
            id: issueRef.id,
            identifier: issueRef.identifier,
    ...
    if (issueRef) {
      ...
      delete context.mercuryIssue;
    }
    if (wakeCommentContext) {
      context.mercuryWakeComment = wakeCommentContext;
    } else {
      delete context.mercuryWakeComment;
    }
    if (taskMarkdown) {
      context.mercuryTaskMarkdown = taskMarkdown;
    } else {
      delete context.mercuryTaskMarkdown;
    }
    ...
    context.mercuryWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.mercuryRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.mercuryRuntimeServiceIntents;
    }
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "Mercury automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
    ...
    return (
      "Mercury automatically retried continuation for this assigned `in_progress` issue during terminal run " +
