...
import type { AdapterRuntimeServiceReport } from "@mercuryai/adapter-utils";
import type { Db } from "@mercuryai/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@mercuryai/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".mercury", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.MERCURY_WORKSPACE_CWD = input.worktreePath;
  env.MERCURY_WORKSPACE_PATH = input.worktreePath;
  env.MERCURY_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.MERCURY_WORKSPACE_BRANCH = input.branchName;
  env.MERCURY_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.MERCURY_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.MERCURY_WORKSPACE_SOURCE = input.base.source;
  env.MERCURY_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.MERCURY_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.MERCURY_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.MERCURY_PROJECT_ID = input.base.projectId ?? "";
  env.MERCURY_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.MERCURY_AGENT_ID = input.agent.id ?? "";
  env.MERCURY_AGENT_NAME = input.agent.name;
  env.MERCURY_COMPANY_ID = input.agent.companyId;
  env.MERCURY_ISSUE_ID = input.issue?.id ?? "";
  env.MERCURY_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.MERCURY_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.MERCURY_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.MERCURY_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.MERCURY_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.MERCURY_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.MERCURY_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.MERCURY_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.MERCURY_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.MERCURY_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.MERCURY_PROJECT_ID = input.workspace.projectId ?? "";
  env.MERCURY_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.MERCURY_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".mercury", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Mercury", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
