...
import type { CreateIssueThreadInteraction, IssueDocumentSummary } from "@mercuryai/shared";
 ...
 *
 * This implementation delegates to the core Mercury domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@mercuryai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
