...
import { environmentLeases } from "@mercuryai/db";
import type {
  Environment,
  EnvironmentLease,
...
import { ensureSshWorkspaceReady, findReachableMercuryApiUrlOverSsh } from "@mercuryai/adapter-utils/ssh";
...
function createSshEnvironmentDriver(db: Db): EnvironmentRuntimeDriver {
  ...
  return {
    ...
    async acquireRunLease(input) {
      ...
      const candidateUrls = (() => {
        const raw = process.env.MERCURY_RUNTIME_API_CANDIDATES_JSON;
        if (!raw) return [];
        try {
      ...
      const mercuryApiUrl = await findReachableMercuryApiUrlOverSsh({
        config: parsed.config,
        candidates: candidateUrls,
      });
      if (!mercuryApiUrl) {
        throw new Error(
          `SSH environment ${parsed.config.username}@${parsed.config.host} could not reach any Mercury API candidates.`,
        );
      }
      return await environmentsSvc.acquireLease({
        companyId: input.companyId,
