 ...
 * `MERCURY_ALLOWED_ATTACHMENT_TYPES` environment variable to a
 * comma-separated list of MIME types or wildcard patterns to expand the
 * allowed set for routes that use this allowlist.
 *
 * Examples:
 *   MERCURY_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf
 *   MERCURY_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf,text/*
 *
 * Supported pattern syntax:
 *   - Exact types:   "application/pdf"
 *   - Wildcards:     "image/*"  or  "application/vnd.openxmlformats-officedocument.*"
 */
import { MAX_COMPANY_ATTACHMENT_MAX_BYTES } from "@mercuryai/shared";
...
const allowedPatterns: string[] = parseAllowedTypes(
  process.env.MERCURY_ALLOWED_ATTACHMENT_TYPES,
);
...
export const MAX_ATTACHMENT_BYTES =
  Number(process.env.MERCURY_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;
