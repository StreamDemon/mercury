...
vi.mock("@mercuryai/shared/telemetry", async () => {
  const actual = await vi.importActual<typeof import("@mercuryai/shared/telemetry")>(
    "@mercuryai/shared/telemetry",
  );
  return {
    ...actual,
...
describeEmbeddedPostgres("active-run output watchdog", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("mercury-active-run-output-watchdog-");
