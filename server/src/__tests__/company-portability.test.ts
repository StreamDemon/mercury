...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "mercury",
        name: "mercury",
        description: "Mercury coordination skill",
        markdown: "---\nname: mercury\ndescription: Mercury coordination skill\n---\n\n# Mercury\n",
        sourceType: "github",
        sourceLocator: "https://github.com/StreamDemon/mercury/tree/master/skills/mercury",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/StreamDemon/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "StreamDemon",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Mercury extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Mercury"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${mercuryKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/StreamDemon/mercury/mercury/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/StreamDemon/mercury/mercury/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/StreamDemon/mercury/mercury/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".mercury.yaml"]);
    expect(extension).toContain('schema: "mercury/v1"');
    ...
    expect(extension).not.toContain("mercurySkillSync");
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".mercury.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });

  it("exports default sidebar order into the Mercury extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".mercury.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/StreamDemon/mercury/mercury/SKILL.md"])).toContain("# Mercury");
    expect(asTextFile(exported.files["skills/StreamDemon/mercury/mercury/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/StreamDemon/mercury/mercury/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/StreamDemon/mercury/mercury/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/StreamDemon/mercury/mercury/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .mercury.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".mercury.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-mercury",
        companyId: "company-1",
        key: "StreamDemon/mercury/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/StreamDemon/mercury/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/StreamDemon/mercury/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/StreamDemon/mercury/release-changelog/SKILL.md"])).toContain("StreamDemon/mercury/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/mercury",
            repoUrl: "https://github.com/StreamDemon/mercury.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .mercury-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/StreamDemon/mercury.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/mercury");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Mercury",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    ...
    const extension = asTextFile(exported.files[".mercury.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/StreamDemon/mercury.git"');
    expect(extension).toContain('projectWorkspaceKey: "mercury"');
  ...
  it("reads env inputs back from .mercury.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .mercury.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Mercury routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "mercury-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Mercury" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Mercury"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .mercury.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Mercury");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".mercury.yaml"] = `${exported.files[".mercury.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
