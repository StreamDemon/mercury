...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Mercury OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/mercury");
    expect(text).toContain("Suggested Mercury base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("mercuryApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.mercuryApiUrl");
    expect(text).toContain("~/.openclaw/workspace/mercury-claimed-api-key.json");
    expect(text).toContain("MERCURY_API_KEY");
