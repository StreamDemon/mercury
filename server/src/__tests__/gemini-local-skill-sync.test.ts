...
describe("gemini local skill sync", () => {
  ...
  it("reports configured Mercury skills and installs them into the Gemini skills home", async () => {
    const home = await makeTempDir("mercury-gemini-skill-sync-");
    ...
    const after = await syncGeminiSkills(ctx, [mercuryKey]);
    expect(after.entries.find((entry) => entry.key === mercuryKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".gemini", "skills", "mercury"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled Mercury skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("mercury-gemini-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncGeminiSkills(configuredCtx, [mercuryKey]);
