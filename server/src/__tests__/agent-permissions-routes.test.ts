...
function registerModuleMocks() {
  vi.doMock("@mercuryai/adapter-opencode-local/server", async () => {
    const actual = await vi.importActual<typeof import("@mercuryai/adapter-opencode-local/server")>("@mercuryai/adapter-opencode-local/server");
    return {
      ...actual,
  ...
  vi.doMock("@mercuryai/shared/telemetry", () => ({
...
describe.sequential("agent permission routes", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@mercuryai/shared/telemetry");
    ...
    vi.doUnmock("@mercuryai/adapter-opencode-local/server");
    registerModuleMocks();
    vi.resetAllMocks();
    ...
    const res = await requestApp(app, (baseUrl) => request(baseUrl)
      ...
      .send({
        adapterConfig: {
          workspaceStrategy: {
            type: "git_worktree",
            provisionCommand: "touch /tmp/mercury-rce",
          },
        },
      }));
