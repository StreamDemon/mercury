...
import { ensureCursorSkillsInjected } from "@mercuryai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Mercury skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("mercury-cursor-skills-src-");
    const skillsHome = await makeTempDir("mercury-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "mercury");
    await createSkillDir(skillsDir, "mercury-create-agent");
    ...
    const injectedA = path.join(skillsHome, "mercury");
    const injectedB = path.join(skillsHome, "mercury-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "mercury")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "mercury-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "mercury"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "mercury-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("mercury-cursor-preserve-src-");
    const skillsHome = await makeTempDir("mercury-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "mercury");
    ...
    expect((await fs.lstat(path.join(skillsHome, "mercury-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("mercury-cursor-fail-src-");
    const skillsHome = await makeTempDir("mercury-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
