...
import type { agents } from "@mercuryai/db";
import { sessionCodec as codexSessionCodec } from "@mercuryai/adapter-codex-local/server";
...
describe("comment wake batching", () => {
  it("preserves ordered wake comment ids when coalescing queued follow-up wakes", () => {
    ...
    expect(merged.mercuryWake).toBeUndefined();
  });
});
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[mercury] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables Mercury-managed rotation by default for codex and claude local", () => {
