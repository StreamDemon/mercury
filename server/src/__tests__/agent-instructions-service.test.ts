...
describe("agent instructions service", () => {
  ...
  const originalMercuryInstanceId = process.env.MERCURY_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalMercuryHome === undefined) delete process.env.MERCURY_HOME;
    else process.env.MERCURY_HOME = originalMercuryHome;
    if (originalMercuryInstanceId === undefined) delete process.env.MERCURY_INSTANCE_ID;
    else process.env.MERCURY_INSTANCE_ID = originalMercuryInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const mercuryHome = await makeTempDir("mercury-agent-instructions-home-");
    const externalRoot = await makeTempDir("mercury-agent-instructions-external-");
    cleanupDirs.add(mercuryHome);
    cleanupDirs.add(externalRoot);
    process.env.MERCURY_HOME = mercuryHome;
    process.env.MERCURY_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("mercury-agent-instructions-new-external-");
    cleanupDirs.add(mercuryHome);
    cleanupDirs.add(externalRoot);
    process.env.MERCURY_HOME = mercuryHome;
    process.env.MERCURY_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("mercury-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const mercuryHome = await makeTempDir("mercury-agent-instructions-recover-");
    cleanupDirs.add(mercuryHome);
    process.env.MERCURY_HOME = mercuryHome;
    process.env.MERCURY_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const mercuryHome = await makeTempDir("mercury-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("mercury-agent-instructions-stale-root-");
    cleanupDirs.add(mercuryHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const mercuryHome = await makeTempDir("mercury-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("mercury-agent-instructions-heal-write-stale-");
    cleanupDirs.add(mercuryHome);
    cleanupDirs.add(staleRoot);
    process.env.MERCURY_HOME = mercuryHome;
    process.env.MERCURY_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const mercuryHome = await makeTempDir("mercury-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("mercury-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(mercuryHome);
    cleanupDirs.add(staleRoot);
    process.env.MERCURY_HOME = mercuryHome;
    process.env.MERCURY_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const mercuryHome = await makeTempDir("mercury-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("mercury-agent-instructions-partial-root-");
    cleanupDirs.add(mercuryHome);
    cleanupDirs.add(staleRoot);
    process.env.MERCURY_HOME = mercuryHome;
    process.env.MERCURY_INSTANCE_ID = "test-instance";
