...
describe("pi local skill sync", () => {
  ...
  it("reports configured Mercury skills and installs them into the Pi skills home", async () => {
    const home = await makeTempDir("mercury-pi-skill-sync-");
    ...
    const after = await syncPiSkills(ctx, [mercuryKey]);
    expect(after.entries.find((entry) => entry.key === mercuryKey)?.state).toBe("installed");
    expect((await fs.lstat(path.join(home, ".pi", "agent", "skills", "mercury"))).isSymbolicLink()).toBe(true);
  });

  it("keeps required bundled Mercury skills installed even when the desired set is emptied", async () => {
    const home = await makeTempDir("mercury-pi-skill-prune-");
    cleanupDirs.add(home);
    ...
    await syncPiSkills(configuredCtx, [mercuryKey]);
