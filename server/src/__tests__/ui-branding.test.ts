...
<head>
    <!-- MERCURY_RUNTIME_BRANDING_START -->
    <!-- MERCURY_RUNTIME_BRANDING_END -->
    <!-- MERCURY_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from MERCURY_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ MERCURY_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ MERCURY_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ MERCURY_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      MERCURY_IN_WORKTREE: "true",
      MERCURY_WORKTREE_NAME: "mercury-pr-432",
      MERCURY_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("mercury-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="mercury-worktree-name"');
    expect(meta).toContain('content="mercury-pr-432"');
    expect(meta).toContain('name="mercury-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="mercury-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="mercury-worktree-name"');
  });
});
