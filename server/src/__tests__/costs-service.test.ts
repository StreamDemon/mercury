...
import { createDb, companies, agents, costEvents, financeEvents, projects } from "@mercuryai/db";
...
describeEmbeddedPostgres("cost and finance aggregate overflow handling", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("mercury-costs-service-");
