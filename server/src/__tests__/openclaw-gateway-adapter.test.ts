...
import { execute, testEnvironment } from "@mercuryai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@mercuryai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("mercury:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("MERCURY_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("MERCURY_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Mercury Wake Payload");
      ...
      expect(payload?.mercury).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
