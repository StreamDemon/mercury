...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "mercury", LOGNAME: "mercury", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "mercury" }),
      },
    );

    expect(tokens).toEqual(["mercury", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "mercury" }, {
        userInfo: () => ({ username: "mercury" }),
      });

      expect(tokens).toEqual(["mercury", "custom-token"]);
    } finally {
