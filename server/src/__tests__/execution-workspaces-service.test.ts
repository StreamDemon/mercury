...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mercury-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Mercury Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@mercury.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("mercury-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `mercury-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "mercury-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "mercury-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "mercury-close-check",
      baseRef: "main",
      metadata: {
