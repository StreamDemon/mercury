...
import type { MercuryPluginManifestV1 } from "@mercuryai/shared";
import {
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("mercury-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["mercury.dbtest", "mercury.escape"]) {
  ...
  async function createPluginPackage(manifest: MercuryPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "mercury-plugin-package-"));
  ...
  async function installPluginRecord(manifest: MercuryPluginManifestV1) {
  ...
  function manifest(pluginKey = "mercury.dbtest"): MercuryPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("mercury.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
