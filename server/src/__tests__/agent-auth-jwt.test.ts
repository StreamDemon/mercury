...
describe("agent local JWT", () => {
  const secretEnv = "MERCURY_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "MERCURY_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "MERCURY_AGENT_JWT_ISSUER";
  const audienceEnv = "MERCURY_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when MERCURY_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "mercury";
    process.env[audienceEnv] = "mercury-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
