...
import type { Issue, MercuryPluginManifestV1 } from "@mercuryai/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: MercuryPluginManifestV1["capabilities"]): MercuryPluginManifestV1 {
  return {
    id: "mercury.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:mercury.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:mercury.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:mercury.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:mercury.test-orchestration");
    await expect(
