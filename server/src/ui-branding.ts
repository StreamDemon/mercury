const FAVICON_BLOCK_START = "<!-- MERCURY_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- MERCURY_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- MERCURY_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- MERCURY_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.MERCURY_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.MERCURY_WORKTREE_NAME) ?? nonEmpty(env.MERCURY_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.MERCURY_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="mercury-worktree-enabled" content="true" />',
    `<meta name="mercury-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="mercury-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="mercury-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
