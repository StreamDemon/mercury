...
const WORKTREE_ENV_HEADER_LINES = [
  ...
  "# Generated by Mercury worktree repair",
];
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.MERCURY_IN_WORKTREE !== "true") return null;

  const configPath = resolveMercuryConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.MERCURY_WORKTREE_NAME) ??
    nonEmpty(env.MERCURY_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.MERCURY_INSTANCE_ID) ??
    nonEmpty(env.MERCURY_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.MERCURY_HOME) ??
      nonEmpty(env.MERCURY_HOME) ??
      nonEmpty(env.MERCURY_WORKTREES_DIR) ??
      "~/.mercury-worktrees",
  );
...
function writeConfigFile(configPath: string, config: MercuryConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".mercury", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as MercuryConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: MercuryConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: MercuryConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.MERCURY_HOME = context.homeDir;
  process.env.MERCURY_INSTANCE_ID = context.instanceId;
  process.env.MERCURY_CONFIG = context.configPath;
  process.env.MERCURY_CONTEXT = context.contextPath;
  process.env.MERCURY_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as MercuryConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    MERCURY_HOME: context.homeDir,
    MERCURY_INSTANCE_ID: context.instanceId,
    MERCURY_CONFIG: context.configPath,
    MERCURY_CONTEXT: context.contextPath,
    MERCURY_IN_WORKTREE: "true",
    MERCURY_WORKTREE_NAME: context.worktreeName,
  };
  ...
  if (repairedEnv) {
    ...
    fs.writeFileSync(
      context.envPath,
      formatEnvEntries(desiredEnvEntries, { headerLines: WORKTREE_ENV_HEADER_LINES }),
      { mode: 0o600 },
    );
  }
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: MercuryConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as MercuryConfig;
  } catch {
    return;
  }
