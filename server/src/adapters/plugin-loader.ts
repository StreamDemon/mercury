...
function extractUiParserSource(
  ...
  const contractVersion = pkg.mercury?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no mercury.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
