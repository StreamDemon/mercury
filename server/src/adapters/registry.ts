...
import { getAdapterSessionManagement } from "@mercuryai/adapter-utils";
import {
...
} from "@mercuryai/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@mercuryai/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@mercuryai/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@mercuryai/adapter-cursor-local";
import {
...
} from "@mercuryai/adapter-gemini-local/server";
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@mercuryai/adapter-gemini-local";
import {
...
} from "@mercuryai/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@mercuryai/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@mercuryai/adapter-openclaw-gateway";
...
import {
  ...
  listPiModels,
} from "@mercuryai/adapter-pi-local/server";
import {
  agentConfigurationDoc as piAgentConfigurationDoc,
} from "@mercuryai/adapter-pi-local";
import {
...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.MERCURY_API_KEY === "string" && existingEnv.MERCURY_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "Mercury API safety rule:",
      "Use Authorization: Bearer $MERCURY_API_KEY on every Mercury API request.",
      "Use X-Mercury-Run-Id: $MERCURY_RUN_ID on every Mercury API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for Mercury API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { MERCURY_API_KEY: normalizedCtx.authToken } : {}),
        MERCURY_RUN_ID: normalizedCtx.runId,
      },
    };
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[mercury] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[mercury] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[mercury] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[mercury] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
