...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@mercuryai/db";
import type { DeploymentMode } from "@mercuryai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  mercuryUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).mercuryUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.mercuryUpgradeContext = context;
