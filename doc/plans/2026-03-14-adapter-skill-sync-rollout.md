...
This document defines the rollout plan for adapter-wide skill support in Mercury.
...
- Mercury stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Mercury currently has these adapters:
...
1. Desired skills live in Mercury for every adapter.
...
These adapters have a stable local skills directory that Mercury can read and manage.
...
These adapters do not have a meaningful Mercury-owned persistent install state.
...
- desired skills stored in Mercury
...
- runtime already injects Mercury skills into `~/.cursor/skills`
...
- runtime already injects Mercury skills into `~/.gemini/skills`
...
- runtime already injects Mercury skills into `~/.pi/agent/skills`
...
- Mercury can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Mercury skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Mercury may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Mercury skills that are clearly marked as Mercury-managed
...
- Mercury stores desired skills only
...
- avoid destructive wording unless Mercury can prove a skill is Mercury-managed
...
That gets Mercury from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
