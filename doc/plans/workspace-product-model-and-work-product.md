...
Mercury needs to support two very different but equally valid ways of working:
...
Today, Mercury already has the beginnings of this model:
...
- making Mercury feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Mercury users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Mercury deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Mercury into a full code review product
...
This object must be recorded explicitly so that Mercury can:
...
Mercury should treat PRs as a type of work product linked back to:
...
Mercury already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Mercury must be a first-class deployment mode

Mercury cannot assume that it is running on the same machine as the code.

In cloud deployments, Mercury may:
...
- `Runtime service`: a process or service Mercury owns or tracks for a workspace
...
It also lets cloud-hosted Mercury deployments point at codebases and remotes without pretending the Mercury host has direct filesystem access.
...
Without an explicit `execution workspace` record, Mercury has nowhere stable to attach:
...
Mercury needs a single place to show:
...
- Mercury-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Mercury owning the execution host
...
## 1. Mercury host and execution host must be decoupled
...
without the Mercury host itself running local git or local preview processes.
...
Mercury should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Mercury.
...
Mercury should keep workspace automation centered on a small set of hooks:
...
Mercury should provide the equivalent visibility in the project `Code` surface:
...
Mercury should treat execution workspaces as the natural home for:
...
Mercury should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Mercury should continue treating cleanup policy as part of the core workspace model:
...
Mercury should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Mercury host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Mercury should borrow Worktrunk's good execution patterns while keeping the broader Mercury model:
...
That keeps Mercury focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Mercury deployments remain first-class

That is a better fit for Mercury than either extreme:
