...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Mercury plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Mercury.
...
- package: `@mercuryai/plugin-kitchen-sink-example`
- plugin id: `mercury.kitchen-sink-example` or `mercury-kitchen-sink-example`

Recommendation: use `mercury-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Mercury event such as `issue.created`
...
Recommendation: default to a Mercury-local endpoint or a stable public echo endpoint to avoid flaky docs.
