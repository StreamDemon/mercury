...
This document defines the next-stage plan for Mercury company import/export.

The core shift is:

- move from a Mercury-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Mercury.
...
Mercury already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Mercury
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `mercury.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.mercury.yaml` sidecar for high-fidelity Mercury-specific details
...
11. adapter-aware skill sync surfaces so Mercury can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Mercury at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Mercury sidecar.
...
Mercury must not redefine `SKILL.md`.
...
- Mercury-specific fidelity lives in `.mercury.yaml`
- Mercury may resolve and install `SKILL.md` packages, but it must not require a Mercury-only skill format
...
### 5.4 Base Package Vs Mercury Extension
...
- base package:
  ...
  - no Mercury-only runtime fields by default
- Mercury extension:
  - `.mercury.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Mercury tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`mercury.manifest.json` is not part of the future package direction.
...
Mercury import/export should support these entity kinds:
...
- Mercury can treat “this set of agents came from team package X” as the imported-team model
...
Mercury should support source references in package metadata with:
...
Mercury should define a new adapter surface area around skills:
...
Planned Mercury behavior:

- if an adapter supports read, Mercury should show current skills in the UI
- if an adapter supports write, Mercury should let the user enable/disable imported skills
- if an adapter supports sync, Mercury should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.mercury.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.mercury.yaml` alongside the base package
...
## 10. Storage Model Inside Mercury
...
Mercury should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `mercuryai company export <company-id> --out <path>`
- `mercuryai company import <path-or-url> --dry-run`
- `mercuryai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Mercury storage, or only referenced at import time?
...
3. Should Mercury support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Mercury aligned with:
