...
Date: 2026-03-13  
Related discussion: https://github.com/StreamDemon/mercury/discussions/449
...
- local runtime data from the default Mercury instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `mercury` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `mercury` skill currently tells agents to do this on essentially every heartbeat:
...
- Mercury discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Mercury skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Mercury-side skill changes land.
...
- `skills/mercury/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/mercury-create-agent/SKILL.md`: 4,718 bytes
...
- Store both:
  ...
  - Mercury-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Mercury to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `mercury` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat Mercury-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `mercury` skill rewrite
...
6. Rewrite `skills/mercury/SKILL.md` around delta-fetch behavior.
