...
This document defines the V1 design for a Mercury adapter that integrates with
...
4. synthesized stdout events for Mercury UI/CLI
...
3. **Agent callback auth to Mercury** uses a bootstrap exchange flow (no long-lived Mercury key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Mercury endpoints, not full SKILL.md prompt injection.
...
- `mercuryPublicUrl` (optional override; else `MERCURY_PUBLIC_URL` env)
...
## Mercury Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `MERCURY_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.mercuryPublicUrl`
2. `process.env.MERCURY_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Mercury credentials in prompt text.

Flow:

1. Before launch/follow-up, Mercury mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Mercury validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Mercury API calls.
...
1. Prompt includes a compact instruction to fetch skills from Mercury.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/mercury`
   - `GET /api/skills/mercury-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <mercuryPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Mercury run context.
...
5. `mercuryPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Mercury skill endpoints |
| Mercury auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Mercury context.
3. Add richer PR metadata surfacing in Mercury UI.
