...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/mercury-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `MERCURY_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `MERCURY_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Mercury may create host-local worktrees, processes, and paths
- `cloud`
  - Mercury should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `mercury | github | vercel | s3 | custom`
...
- use `MERCURY_EXECUTION_TOPOLOGY` as a defaulting guardrail
