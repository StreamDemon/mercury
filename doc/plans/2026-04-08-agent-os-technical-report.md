# Agent OS Technical Report for Mercury
...
`agent-os` is not a competitor to Mercury's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. Mercury is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that Mercury could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in Mercury.

My recommendation is:

1. Do not merge agent-os concepts into the Mercury core product model.
...
4. Do not import its workflow/cron/queue abstractions into Mercury core until they are reconciled with Mercury's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than Mercury's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to Mercury because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## Mercury anchor points

The most relevant current Mercury surfaces for any future `agent-os` integration are:
...
## What Mercury can learn from it

### 1. A safer local execution substrate

Mercury's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits Mercury because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

Mercury already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
Mercury has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that Mercury could adopt even without adopting the runtime itself:
...
Mercury's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.

Concrete Mercury uses:
...
This aligns well with Mercury's governance story.

### 5. Better adapter normalization around sessions and capabilities

Mercury's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
Mercury does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
Mercury can adopt that philosophy directly:
...
## What does not fit Mercury well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. Mercury already has higher-level orchestration concepts:
...
If Mercury copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

Mercury should keep orchestration authoritative at the control-plane layer.
...
Those are Mercury's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. Mercury would still need to answer:
...
- keep Mercury heartbeat/task/workspace/budget logic unchanged
...
- Mercury's Codex adapter is already important and carries repo-specific behavior
...
- Mercury still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, Mercury should consider standardizing adapter/runtime permissions around a vocabulary like:
...
Mercury should evaluate whether some execution workspaces can be backed by:
...
Mercury plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into Mercury core

Mercury should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving Mercury orchestration into agent-os workflows
...
`agent-os` is useful to Mercury as an execution technology reference, not as a product model.

Mercury should treat it the same way it treats sandboxes or agent CLIs:
