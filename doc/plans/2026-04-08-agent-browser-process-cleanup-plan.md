...
Yes, there is a likely root cause in Mercury's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Mercury can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Mercury already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Mercury may think it cancelled the run because the parent process is gone
...
This work is successful when Mercury can:
...
- require every skill author to add bespoke browser teardown logic before Mercury can clean up correctly
...
- make the leak measurable from Mercury's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Mercury
...
Treat this as a Mercury executor ownership bug, not an `agent-browser` bug.
