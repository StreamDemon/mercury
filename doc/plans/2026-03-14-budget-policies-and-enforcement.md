...
Mercury already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Mercury should implement next.

## Product Goals

Mercury should let operators:
...
For the next implementation pass, Mercury should enforce these defaults:
...
Mercury should separate subscription-included usage from billed spend:
...
Mercury should have two threshold classes:
...
Mercury must know that a project is budget-paused and enforce it.
...
Mercury must also block execution before new work starts.
