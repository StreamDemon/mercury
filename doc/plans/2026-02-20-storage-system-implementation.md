...
Add a single storage subsystem for Mercury that supports:
...
- API serves attachment bytes through authenticated Mercury endpoints.
...
- Default local path is under instance root: `~/.mercury/instances/<instanceId>/data/storage`.
...
- `mercuryai onboard` writes a valid `storage` config block by default.
- `mercuryai configure --section storage` can switch between local and s3 modes.
...
- `mercuryai doctor` reports actionable storage status.
