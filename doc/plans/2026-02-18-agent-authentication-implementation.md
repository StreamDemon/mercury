...
- New config/env settings:
  - `MERCURY_AGENT_JWT_SECRET`
  - `MERCURY_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `MERCURY_AGENT_JWT_ISSUER` (default: `mercury`)
  - `MERCURY_AGENT_JWT_AUDIENCE` (default: `mercury-api`)
...
1. In:
   ...
   inject `MERCURY_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `MERCURY_API_KEY`, do not overwrite it.
- Continue injecting:
  - `MERCURY_AGENT_ID`
  - `MERCURY_COMPANY_ID`
  - `MERCURY_API_URL`
...
- Local adapters authenticate without manual `MERCURY_API_KEY` config.
