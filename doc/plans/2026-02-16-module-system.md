...
Mercury's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Mercury's API, UI, and data model | Yes |
...
modules/
  observability/
    mercury.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@mercuryai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@mercuryai/core";
...
```
1. Scan modules/ directory for mercury.module.json manifests
...
```jsonc
// mercury.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@mercuryai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@mercuryai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:mercury/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.mercury.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm mercuryai store list                    # browse available modules and templates
pnpm mercuryai store install <module-id>     # install a module
pnpm mercuryai store import <template-id>    # import a company template
pnpm mercuryai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Mercury instance | `server:started` |
...
4. **Module config** — `mercury.config.json` with per-module config, env var interpolation
...
10. **`@mercuryai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
