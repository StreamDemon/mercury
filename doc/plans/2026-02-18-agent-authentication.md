...
Agents need API keys to authenticate with Mercury. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Mercury
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Mercury generates a token and passes it directly to the agent
...
`MERCURY_API_URL`, `MERCURY_AGENT_ID`, `MERCURY_COMPANY_ID`. We need to
add a `MERCURY_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Mercury CLI
...
```
mercury auth login
# Opens browser -> user confirms -> token stored at ~/.mercury/credentials
```
...
managed by the Mercury server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Mercury.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Mercury SKILL.md (or a link to it)
   - What information Mercury needs from the agent (e.g. webhook URL, adapter
...
5. Mercury stores the pending registration.
...
7. On approval, Mercury provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Mercury issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.mercury.ing/skills/mercury/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Mercury should send you heartbeats",
...
1. Generate a Mercury invite link for the company.
...
4. A Mercury company member approves the new agent.
5. Mercury begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `MERCURY_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `mercuryai auth login` for developer-managed remote agents.                                      |
