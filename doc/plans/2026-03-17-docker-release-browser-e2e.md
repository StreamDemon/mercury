...
Today release smoke testing for published Mercury packages is manual and shell-driven:
...
- Docker
- `npx mercuryai@canary`
- `npx mercuryai@latest`
...
- runs `mercuryai@${MERCURYAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@mercury.local`
- password: `mercury-smoke-password`
...
- the suite passes locally against both:
  - `MERCURYAI_VERSION=canary`
  - `MERCURYAI_VERSION=latest`
...
1. A published `mercuryai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `mercuryai@latest` Docker install can be smoke-tested by Playwright in CI.
