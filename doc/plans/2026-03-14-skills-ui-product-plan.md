...
This document defines the product and UI plan for skill management in Mercury.
...
- Mercury company import/export can include skills as package content
...
3. The UI must always tell the truth about what Mercury knows:
   - desired state in Mercury
...
Mercury should treat skills at two scopes:
...
- created locally inside Mercury later
...
7. “Understand what Mercury knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Mercury-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Mercury did not install.

These should be clearly marked:

- external
- not managed by Mercury
...
- `desired_only`: Mercury wants it, adapter does not show it yet
- `external`: adapter has it but Mercury does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Mercury only tracks desired state
...
- Mercury can still track desired skills
...
- Mercury can see adapter skills
...
- `.mercury.yaml` may add Mercury-specific fidelity, but should not replace the base shortname association model
...
- `.mercury.yaml` may add Mercury-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Mercury should have a company-scoped skill model or managed package model representing:
...
Mercury should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Mercury-specific.
...
That gives Mercury one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
