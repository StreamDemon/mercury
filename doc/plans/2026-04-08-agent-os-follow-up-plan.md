...
Turn the `agent-os` research into a low-risk Mercury execution plan that preserves Mercury's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Mercury should not absorb `agent-os` as a product model or orchestration layer.

Mercury should evaluate `agent-os` in three narrow areas:
...
This work is successful when Mercury has:
...
- a proposed runtime capability model that fits current Mercury adapters
...
- replace Mercury heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Mercury core

## Existing Mercury integration points
...
- prove that Mercury can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Mercury's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Mercury-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Mercury workloads
...
- identify where Mercury should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Mercury
...
- runtime experimentation could blur the boundary between Mercury as control plane and Mercury as execution platform
