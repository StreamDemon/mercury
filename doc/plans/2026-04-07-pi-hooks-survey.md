...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Mercury.
...
- current Mercury plugin and adapter surfaces in this repo
...
## What Mercury already has

Mercury already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Mercury

These ideas from `pi-mono` fit Mercury with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Mercury should continue exposing run and transcript events to plugins, for example:
...
This matches Mercury's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Mercury already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Mercury already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Mercury, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Mercury core:

### 1. Arbitrary mutation hooks on control-plane decisions

Mercury should not let general plugins rewrite:
...
Mercury should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Mercury is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Mercury's board-and-issues model.

## Recommended Mercury direction
...
If the question is "what works for Mercury too?":
