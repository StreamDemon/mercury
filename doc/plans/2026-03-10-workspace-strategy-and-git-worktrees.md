...
`PAP-447` asks how Mercury should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Mercury instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Mercury:

- some operators use Mercury to manage Mercury and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Mercury
- Claude and Codex expose different built-in affordances, so Mercury should not overfit to one tool

## Core Product Decision

Mercury should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Mercury
...
- Mercury should not assume that every adapter can see or use a host filesystem path directly
...
If Mercury makes worktrees agent-first, it will blur:
...
- Mercury core resolves and records execution workspace state
...
That gives Mercury room to support:
...
- Mercury handles workspace runtime services under the hood using project/workspace policy
...
Once Mercury is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Mercury cannot infer
...
- when issue moves to done, should Mercury auto-commit?
- should it auto-push?
...
Mercury already has the right foundation for a project-first model.
...
For server-spawned local adapters, Mercury already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Mercury should mirror directly

Implication:

- for `codex_local`, Mercury should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Mercury should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Mercury should separate:
...
Mercury should be able to express intentions such as:
...
These are not specific to Mercury. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".mercury/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Mercury-specific `devServer` flag.
...
Mercury should distinguish between:
...
Mercury should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Mercury should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Mercury
...
For runtime services specifically, Mercury should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Mercury-only dev-server flags
...
3. Inject resolved execution workspace details into `context.mercuryWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Mercury instance can be reused through env/config injection when appropriate
...
- Mercury can decide whether to reuse or start a fresh service deterministically
...
3. Test against Mercury-on-Mercury workflows first.
...
7. Workspace runtime services are modeled generically, not as Mercury-specific dev-server toggles.
