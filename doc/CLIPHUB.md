...
ClipHub is the public registry where people share, discover, and download Mercury company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Mercury what a package registry is to a programming language. Mercury already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Mercury — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Mercury instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Mercury spec. It contains:
...
**Install (fresh start):**
```
mercury install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Mercury instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Mercury, export your company as a template, then publish:

```
mercury export --template my-company
mercury publish cliphub my-company
```
...
ClipHub is a **separate service** from Mercury itself. Mercury is self-hosted; ClipHub is a hosted registry that Mercury instances talk to.
...
| **Mercury CLI** | `mercuryai install`, `mercuryai publish`, `mercuryai cliphub sync` — built into Mercury |
| **Mercury UI** | "Browse ClipHub" panel in the Mercury web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Mercury) |
| Backend | TypeScript + Hono (consistent with Mercury) |
| Database | PostgreSQL |
...
4. Run `mercuryai install cliphub:acme/lean-saas-shop`
5. Mercury creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Mercury until it works well
2. Export: `mercuryai export --template my-agency`
3. Publish: `mercuryai publish cliphub my-agency`
...
3. Install just that agent: `mercuryai install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use Mercury. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Mercury what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `mercuryai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Mercury web UI
- [ ] `mercuryai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Mercury's own philosophy)
