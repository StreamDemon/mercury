...
For `claude_local` and `codex_local`, Mercury assumes the CLI is already installed and authenticated on the host machine.
...
Mercury stores resumable session state per `(agent, taskKey, adapterType)`.
