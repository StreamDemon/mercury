...
This spec defines how Mercury actually runs agents while staying runtime-agnostic.
...
1. Mercury is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Mercury manages lifecycle, and on exit Mercury parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `mercury.skill` (shared Mercury skill text block)
...
2. Preferred transport is env vars (`MERCURY_*`) injected at runtime.
