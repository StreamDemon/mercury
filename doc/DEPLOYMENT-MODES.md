...
Mercury supports two runtime modes:
...
Mercury now treats **bind** as a separate concern from auth:
...
pnpm mercuryai onboard --yes --bind lan
pnpm mercuryai run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Mercury emits a startup warning with a one-time high-entropy claim URL.
