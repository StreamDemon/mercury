...
Low-level reference for how Mercury packages are prepared and published to npm.
...
Mercury no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `mercuryai`, imports code from workspace packages such as:
...
### Manual first publish for `@mercuryai/ui`
...
npm view @mercuryai/ui version

# make sure the dist payload is fresh
pnpm --filter @mercuryai/ui build
...
- If `npm view @mercuryai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@mercuryai/ui`, the `mercuryai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Mercury uses calendar versions:
