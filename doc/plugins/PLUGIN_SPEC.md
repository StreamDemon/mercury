# Mercury Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Mercury's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Mercury app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Mercury HTTP APIs directly.
...
Mercury plugin design is based on the following assumptions:

1. Mercury is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Mercury business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Mercury core.
...
2. Let plugins add major capabilities without editing Mercury core.
...
4. Depend on project-local plugin folders such as `.mercury/plugins`.
...
The single Mercury deployment an operator installs and controls.

### 5.2 Company

A first-class Mercury business object inside the instance.
...
A trusted in-process extension loaded directly by Mercury core.
...
An installable instance-wide extension package loaded through the Mercury plugin runtime.
...
Mercury has two extension classes.
...
Mercury already has a concrete workspace model:
...
Plugins live under the Mercury instance directory.

Suggested layout:

- `~/.mercury/instances/default/plugins/package.json`
- `~/.mercury/instances/default/plugins/node_modules/`
- `~/.mercury/instances/default/plugins/.cache/`
- `~/.mercury/instances/default/data/plugins/<plugin-id>/`
...
Mercury should add CLI commands:

- `pnpm mercuryai plugin list`
- `pnpm mercuryai plugin install <package[@version]>`
- `pnpm mercuryai plugin uninstall <plugin-id>`
- `pnpm mercuryai plugin upgrade <plugin-id> [version]`
- `pnpm mercuryai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@mercury/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface MercuryPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Mercury agents can use during runs.
...
- Mercury server starts one worker process per installed plugin
...
Receives one typed Mercury domain event.
...
export function definePlugin(definition: PluginDefinition): MercuryPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: MercuryPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@mercuryai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@mercuryai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@mercuryai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@mercuryai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@mercuryai/plugin-sdk/ui`
...
The `@mercuryai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Mercury secret provider system rather than a plain text input
...
1. Core Mercury data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Mercury objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Mercury product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Mercury later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Mercury secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm mercuryai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Mercury server. This is a normative requirement, not optional.
...
### 27.1 `@mercuryai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@mercuryai/plugin-test-harness";
...
For developing a plugin against a running Mercury instance:

- The operator installs the plugin from a local path: `pnpm mercuryai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-mercury-plugin`) that scaffolds:

- `package.json` with correct `mercuryPlugin` keys
...
- `@mercury/plugin-workspace-files`
- `@mercury/plugin-terminal`
- `@mercury/plugin-git`
- `@mercury/plugin-linear`
- `@mercury/plugin-github-issues`
- `@mercury/plugin-grafana`
- `@mercury/plugin-runtime-processes`
- `@mercury/plugin-stripe`
...
- `@mercuryai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@mercuryai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@mercuryai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@mercuryai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@mercuryai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@mercuryai/plugin-sdk/ui`
...
- `@mercuryai/plugin-test-harness`
- `create-mercury-plugin` starter template
...
Mercury should not implement a generic in-process hook bag modeled directly after local coding tools.

Mercury should implement:
...
That is the complete target design for the Mercury plugin system.
