...
This guide describes the current, implemented way to create a Mercury plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Mercury app.
...
```bash
pnpm --filter @mercuryai/create-mercury-plugin build
node packages/plugins/create-mercury-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Mercury repo:

```bash
pnpm --filter @mercuryai/create-mercury-plugin build
node packages/plugins/create-mercury-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@mercuryai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@mercuryai/plugin-sdk` from the local Mercury checkout into a `.mercury-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Mercury from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@mercuryai/plugin-sdk/ui`
