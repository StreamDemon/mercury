...
Use when:
- You want Mercury to run Pi (the AI coding agent) locally as the agent runtime
...
- Mercury requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/mercurys/ and resumed with --session.
