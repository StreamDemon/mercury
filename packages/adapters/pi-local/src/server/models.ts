...
import type { AdapterModel } from "@mercuryai/adapter-utils";
import { asString, runChildProcess } from "@mercuryai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.MERCURY_PI_COMMAND === "string" &&
    process.env.MERCURY_PI_COMMAND.trim().length > 0
      ? process.env.MERCURY_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["MERCURY_", "npm_", "NPM_"] as const;
