...
const MERCURY_SESSIONS_DIR = path.join(os.homedir(), ".pi", "mercurys");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[mercury] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[mercury] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[mercury] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildPiSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "mercury-pi-skills-"));
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(MERCURY_SESSIONS_DIR, { recursive: true });
  return MERCURY_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(MERCURY_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const piSkillEntries = await readMercuryRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveMercuryDesiredSkillNames(config, piSkillEntries);
  if (!executionTargetIsRemote) {
  ...
  // invoke skill binaries (e.g. `mercury-get-issue`) by name. Without this,
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[mercury] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      ...
      executionTargetIsRemote
        ? `[mercury] Pi session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`
        : `[mercury] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[mercury] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !canResumeSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderMercuryWakePrompt(context.mercuryWake, { resumedSession: canResumeSession });
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[mercury] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
      );
