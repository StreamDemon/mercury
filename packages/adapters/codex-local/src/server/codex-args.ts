import { asBoolean, asString, asStringArray } from "@mercuryai/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Mercury will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
