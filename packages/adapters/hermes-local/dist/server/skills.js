...
import { readMercuryRuntimeSkillEntries, resolveMercuryDesiredSkillNames, } from "@mercuryai/adapter-utils/server-utils";
...
async function buildSkillEntry(key, skillMdPath, categoryPath) {
    ...
    return {
        ...
        readOnly: true, // Hermes manages its own skills — Mercury can't toggle them
...
async function buildHermesSkillSnapshot(config) {
    ...
    // 1. Scan Mercury-managed skills (bundled with the adapter)
    const mercuryEntries = await readMercuryRuntimeSkillEntries(config, __moduleDir);
    const desiredSkills = resolveMercuryDesiredSkillNames(config, mercuryEntries);
    const desiredSet = new Set(desiredSkills);
    const availableByKey = new Map(mercuryEntries.map((e) => [e.key, e]));
    ...
    // 3. Merge: Mercury skills first (ephemeral), then Hermes skills
    const entries = [];
    const warnings = [];
    // Mercury-managed skills
    for (const entry of mercuryEntries) {
        ...
        entries.push({
            ...
            origin: entry.required ? "mercury_required" : "company_managed",
            originLabel: entry.required ? "Required by Mercury" : "Managed by Mercury",
            readOnly: false,
    ...
    for (const entry of hermesSkillEntries) {
        // Skip if Mercury already manages a skill with the same key
    ...
    for (const desiredSkill of desiredSkills) {
        if (availableByKey.has(desiredSkill) || hermesKeys.has(desiredSkill))
            continue;
        warnings.push(`Desired skill "${desiredSkill}" is not available in Mercury or Hermes skills.`);
        entries.push({
            key: desiredSkill,
            runtimeName: null,
            desired: true,
            ...
            sourcePath: null,
            targetPath: null,
            detail: "Cannot find this skill in Mercury or ~/.hermes/skills/.",
        });
    }
    return {
...
export function resolveHermesDesiredSkillNames(config, availableEntries) {
    return resolveMercuryDesiredSkillNames(config, availableEntries);
}
