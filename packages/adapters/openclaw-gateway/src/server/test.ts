...
import { asString, parseObject } from "@mercuryai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Mercury server host.",
        });
      }
    } catch (err) {
      checks.push({
