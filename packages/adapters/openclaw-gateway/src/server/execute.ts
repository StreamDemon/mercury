...
const DEFAULT_CLIENT_VERSION = "mercury";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "mercury";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`mercury:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`mercury:issue:${input.issueId}`, input.agentId);
  }
...
function resolveMercuryApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/mercury-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildMercuryEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const mercuryApiUrlOverride = resolveMercuryApiUrlOverride(ctx.config.mercuryApiUrl);
  const mercuryEnv: Record<string, string> = {
    ...buildMercuryEnv(ctx.agent),
    MERCURY_RUN_ID: ctx.runId,
  };

  if (mercuryApiUrlOverride) {
    mercuryEnv.MERCURY_API_URL = mercuryApiUrlOverride;
  }
  if (wakePayload.taskId) mercuryEnv.MERCURY_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) mercuryEnv.MERCURY_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) mercuryEnv.MERCURY_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) mercuryEnv.MERCURY_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) mercuryEnv.MERCURY_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    mercuryEnv.MERCURY_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  mercuryEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/mercury-claimed-api-key.json";
  const orderedKeys = [
    "MERCURY_RUN_ID",
    "MERCURY_AGENT_ID",
    "MERCURY_COMPANY_ID",
    ...
    "MERCURY_TASK_ID",
    "MERCURY_WAKE_REASON",
    "MERCURY_WAKE_COMMENT_ID",
    "MERCURY_APPROVAL_ID",
    "MERCURY_APPROVAL_STATUS",
    "MERCURY_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = mercuryEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = mercuryEnv.MERCURY_API_URL ?? "<set MERCURY_API_URL>";

  const lines = [
    "Mercury wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `MERCURY_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load MERCURY_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $MERCURY_API_KEY on every API call.",
    "- Use X-Mercury-Run-Id: $MERCURY_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: MERCURY_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$MERCURY_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$MERCURY_COMPANY_ID/issues?assigneeAgentId=$MERCURY_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardMercuryPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  mercuryEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateMercury = parseObject(payloadTemplate.mercury);
  const workspace = asRecord(ctx.context.mercuryWorkspace);
  const workspaces = Array.isArray(ctx.context.mercuryWorkspaces)
    ? ctx.context.mercuryWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.mercuryRuntimeServiceIntents)
    ? ctx.context.mercuryRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardMercury: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: mercuryEnv.MERCURY_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.mercuryWake);
  if (Object.keys(structuredWake).length > 0) {
    standardMercury.wake = structuredWake;
  }

  if (workspace) {
    standardMercury.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardMercury.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardMercury.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "mercury-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const mercuryEnv = buildMercuryEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderMercuryWakePrompt(ctx.context.mercuryWake);
  const structuredWakeJson = stringifyMercuryWakePayload(ctx.context.mercuryWake);
  ...
  const mercuryPayload = buildStandardMercuryPayload(ctx, wakePayload, mercuryEnv, payloadTemplate);
  ...
  agentParams.mercury = mercuryPayload;
