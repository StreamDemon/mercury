...
Use when:
- You want Mercury to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Mercury server.
...
- mercuryApiUrl (string, optional): absolute Mercury base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/mercury-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default mercury)

Standard outbound payload additions:
- mercury (object): standardized Mercury context added to every gateway agent request
- mercury.workspace (object, optional): resolved execution workspace for this run
- mercury.workspaces (array, optional): additional workspace hints Mercury exposed to the run
- mercury.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
