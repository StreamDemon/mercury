...
## Scope
This plan is now **gateway-only**. Mercury supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Mercury skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Mercury task.
...
```bash
pnpm --filter @mercuryai/server test:run -- openclaw-gateway
pnpm --filter @mercuryai/server typecheck
pnpm --filter @mercuryai/ui typecheck
pnpm --filter mercuryai typecheck
```
