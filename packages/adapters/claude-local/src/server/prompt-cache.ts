...
import type { AdapterExecutionContext } from "@mercuryai/adapter-utils";
import { ensureMercurySkillSymlink, type MercurySkillEntry } from "@mercuryai/adapter-utils/server-utils";

const DEFAULT_MERCURY_INSTANCE_ID = "default";

type SkillEntry = MercurySkillEntry;
...
function resolveManagedClaudePromptCacheRoot(
  ...
  const mercuryHome = nonEmpty(env.MERCURY_HOME) ?? path.resolve(os.homedir(), ".mercury");
  const instanceId = nonEmpty(env.MERCURY_INSTANCE_ID) ?? DEFAULT_MERCURY_INSTANCE_ID;
  return path.resolve(
    mercuryHome,
    "instances",
    instanceId,
...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("mercury-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      await ensureMercurySkillSymlink(entry.source, target);
    } catch (err) {
      await onLog(
        "stderr",
        `[mercury] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
