...
Use when:
- You want Mercury to run Cursor Agent CLI locally as the agent runtime
...
- Mercury auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$mercury" and related skills on local runs.
- Mercury auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
