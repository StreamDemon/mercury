...
async function getIssueWorkspaceRuntime(client: MercuryApiClient, issueId: string) {
...
export function createToolDefinitions(client: MercuryApiClient): ToolDefinition[] {
  return [
    makeTool(
      "mercuryQS",
      "Get the current authenticated Mercury actor details",
      z.object({}),
    ...
    makeTool(
      "mercuryApiRequest",
      "Make a JSON request to an existing Mercury /api endpoint for unsupported operations",
      apiRequestSchema,
