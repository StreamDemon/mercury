...
describe("renderMercuryWakePrompt", () => {
  it("keeps the default local-agent prompt action-oriented", () => {
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("Start actionable work in this heartbeat");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("do not stop at a plan");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("Prefer the smallest verification that proves the change");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("Use child issues");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("instead of polling agents, sessions, or processes");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("Create child issues directly when you know what needs to be done");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("POST /api/issues/{issueId}/interactions");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("kind suggest_tasks, ask_user_questions, or request_confirmation");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("confirmation:{issueId}:plan:{revisionId}");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain("Wait for acceptance before creating implementation subtasks");
    expect(DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE).toContain(
      "Respect budget, pause/cancel, approval gates, and company boundaries",
    );
  });

  it("adds the execution contract to scoped wake prompts", () => {
    const prompt = renderMercuryWakePrompt({
    ...
    expect(prompt).toContain("## Mercury Wake Payload");
  ...
  it("includes continuation and child issue summaries in structured wake context", () => {
    ...
    expect(JSON.parse(stringifyMercuryWakePayload(payload) ?? "{}")).toMatchObject({
      continuationSummary: {
    ...
    const prompt = renderMercuryWakePrompt(payload);
...
describe("applyMercuryWorkspaceEnv", () => {
  it("adds shared workspace env vars including AGENT_HOME", () => {
    const env = applyMercuryWorkspaceEnv(
      {},
      {
        ...
        workspaceRepoUrl: "https://github.com/StreamDemon/mercury.git",
    ...
    expect(env).toEqual({
      MERCURY_WORKSPACE_CWD: "/tmp/workspace",
      MERCURY_WORKSPACE_SOURCE: "project_primary",
      MERCURY_WORKSPACE_STRATEGY: "git_worktree",
      MERCURY_WORKSPACE_ID: "workspace-1",
      MERCURY_WORKSPACE_REPO_URL: "https://github.com/StreamDemon/mercury.git",
      MERCURY_WORKSPACE_REPO_REF: "main",
      MERCURY_WORKSPACE_BRANCH: "feature/test",
      MERCURY_WORKSPACE_WORKTREE_PATH: "/tmp/worktree",
