...
export function parseSandboxRemoteExecutionSpec(value: unknown): SandboxRemoteExecutionSpec | null {
  ...
  return {
    ...
    mercuryApiUrl: asString(parsed.mercuryApiUrl).trim() || null,
  };
}
...
export function buildSandboxExecutionSessionIdentity(spec: SandboxRemoteExecutionSpec | null) {
  ...
  return {
    ...
    ...(spec.mercuryApiUrl ? { mercuryApiUrl: spec.mercuryApiUrl } : {}),
  } as const;
}
...
export function sandboxExecutionSessionMatches(saved: unknown, current: SandboxRemoteExecutionSpec | null): boolean {
  ...
  return (
    ...
    asString(parsedSaved.mercuryApiUrl) === asString(currentIdentity.mercuryApiUrl)
  );
}
...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".mercury-runtime", input.adapterKey);

  await withTempDir("mercury-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".mercury-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("mercury-sandbox-restore-", async (tempDir) => {
        ...
        await mirrorDirectory(extractedDir, input.workspaceLocalDir, {
          preserveAbsent: [".mercury-runtime", ...(input.preserveAbsentOnRestore ?? [])],
        });
      });
    },
  };
