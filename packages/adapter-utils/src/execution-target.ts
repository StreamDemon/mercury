...
export function adapterExecutionTargetMercuryApiUrl(
  target: AdapterExecutionTarget | null | undefined,
): string | null {
  if (target?.kind !== "remote") return null;
  if (target.transport === "ssh") return target.mercuryApiUrl ?? target.spec.mercuryApiUrl ?? null;
  return target.mercuryApiUrl ?? null;
}
...
export function adapterExecutionTargetSessionIdentity(
  ...
  return {
    ...
    ...(target.mercuryApiUrl ? { mercuryApiUrl: target.mercuryApiUrl } : {}),
  };
}
...
export function adapterExecutionTargetSessionMatches(
  ...
  return (
    ...
    readStringMeta(parsedSaved, "mercuryApiUrl") === (current?.mercuryApiUrl ?? null)
  );
}
...
export function parseAdapterExecutionTarget(value: unknown): AdapterExecutionTarget | null {
  ...
  if (kind === "remote" && readStringMeta(parsed, "transport") === "ssh") {
    ...
    return {
      ...
      mercuryApiUrl: readStringMeta(parsed, "mercuryApiUrl") ?? spec.mercuryApiUrl ?? null,
      spec,
    };
  }
  ...
  if (kind === "remote" && readStringMeta(parsed, "transport") === "sandbox") {
    ...
    return {
      ...
      leaseId: readStringMeta(parsed, "leaseId"),
      remoteCwd,
      mercuryApiUrl: readStringMeta(parsed, "mercuryApiUrl"),
...
export function adapterExecutionTargetFromRemoteExecution(
  ...
  if (ssh) {
    return {
      ...
      mercuryApiUrl: ssh.mercuryApiUrl ?? null,
      spec: ssh,
    };
  }
...
export async function prepareAdapterExecutionTargetRuntime(input: {
  ...
  const prepared = await prepareCommandManagedRuntime({
    ...
    spec: {
      ...
      mercuryApiUrl: target.mercuryApiUrl,
    },
    adapterKey: input.adapterKey,
    workspaceLocalDir: input.workspaceLocalDir,
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".mercury-runtime", key);
}
