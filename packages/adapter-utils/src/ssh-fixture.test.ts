...
describe("ssh env-lab fixture", () => {
  ...
  it("starts an isolated sshd fixture and executes commands through it", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "mercury-ssh-fixture-"));
    ...
    await expect(
      buildSshSpawnTarget({
        spec: {
          ...
          port: 22,
          username: "ssh-user",
          remoteCwd: "/srv/mercury/workspace",
  ...
  it("syncs a local directory into the remote fixture workspace", async () => {
    ...
    await writeFile(path.join(localDir, "message.txt"), "hello from mercury\n", "utf8");
    ...
    expect(result.stdout).toContain("hello from mercury");
  ...
  it("round-trips a git workspace through the SSH fixture", async () => {
    ...
    await git(localRepo, ["config", "user.name", "Mercury Test"]);
    await git(localRepo, ["config", "user.email", "test@mercury.dev"]);
    ...
    await runSshCommand(
      config,
      `sh -lc 'cd ${JSON.stringify(started.workspaceDir)} && git config user.name "Mercury SSH" && git config user.email "ssh@mercury.dev" && git add tracked.txt untracked.txt && git commit -m "remote update" >/dev/null && printf "remote dirty\\n" > tracked.txt && printf "remote extra\\n" > remote-only.txt'`,
