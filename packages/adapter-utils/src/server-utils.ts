...
const MERCURY_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export const DEFAULT_MERCURY_AGENT_PROMPT_TEMPLATE = [
  "You are agent {{agent.id}} ({{agent.name}}). Continue your Mercury work.",
  "",
  "Execution contract:",
...
export interface MercurySkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: MercurySkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "mercury_required",
      originLabel: "Required by Mercury",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Mercury",
    readOnly: false,
  };
}
...
type MercuryWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type MercuryWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: MercuryWakeExecutionPrincipal | null;
  returnAssignee: MercuryWakeExecutionPrincipal | null;
  reviewRequest: {
    instructions: string;
  } | null;
...
type MercuryWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type MercuryWakeLivenessContinuation = {
  attempt: number | null;
...
type MercuryWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type MercuryWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type MercuryWakeTreeHoldSummary = {
  holdId: string | null;
...
type MercuryWakePayload = {
  reason: string | null;
  issue: MercuryWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  treeHoldInteraction: boolean;
  activeTreeHold: MercuryWakeTreeHoldSummary | null;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: MercuryWakeBlockerSummary[];
  executionStage: MercuryWakeExecutionStage | null;
  continuationSummary: MercuryWakeContinuationSummary | null;
  livenessContinuation: MercuryWakeLivenessContinuation | null;
  childIssueSummaries: MercuryWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: MercuryWakeComment[];
...
function normalizeMercuryWakeIssue(value: unknown): MercuryWakeIssue | null {
...
function normalizeMercuryWakeComment(value: unknown): MercuryWakeComment | null {
...
function normalizeMercuryWakeContinuationSummary(value: unknown): MercuryWakeContinuationSummary | null {
...
function normalizeMercuryWakeLivenessContinuation(value: unknown): MercuryWakeLivenessContinuation | null {
...
function normalizeMercuryWakeChildIssueSummary(value: unknown): MercuryWakeChildIssueSummary | null {
...
function normalizeMercuryWakeBlockerSummary(value: unknown): MercuryWakeBlockerSummary | null {
...
function normalizeMercuryWakeTreeHoldSummary(value: unknown): MercuryWakeTreeHoldSummary | null {
...
function normalizeMercuryWakeExecutionPrincipal(value: unknown): MercuryWakeExecutionPrincipal | null {
...
function normalizeMercuryWakeExecutionStage(value: unknown): MercuryWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeMercuryWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeMercuryWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeMercuryWakePayload(value: unknown): MercuryWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeMercuryWakeComment(entry))
        .filter((entry): entry is MercuryWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeMercuryWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizeMercuryWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizeMercuryWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizeMercuryWakeChildIssueSummary(entry))
        .filter((entry): entry is MercuryWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizeMercuryWakeBlockerSummary(entry))
        .filter((entry): entry is MercuryWakeBlockerSummary => Boolean(entry))
    : [];

  const activeTreeHold = normalizeMercuryWakeTreeHoldSummary(payload.activeTreeHold);
  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !livenessContinuation && !normalizeMercuryWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeMercuryWakeIssue(payload.issue),
...
export function stringifyMercuryWakePayload(value: unknown): string | null {
  const normalized = normalizeMercuryWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function renderMercuryWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: MercuryWakeExecutionPrincipal | null) => {
  ...
  const lines = resumedSession
      ? [
        "## Mercury Resume Delta",
        "",
        "You are resuming an existing Mercury session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "MERCURY_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildMercuryEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    MERCURY_AGENT_ID: agent.id,
    MERCURY_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.MERCURY_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.MERCURY_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl =
    process.env.MERCURY_RUNTIME_API_URL ??
    process.env.MERCURY_API_URL ??
    `http://${runtimeHost}:${runtimePort}`;
  vars.MERCURY_API_URL = apiUrl;
  return vars;
}

export function applyMercuryWorkspaceEnv(
  env: Record<string, string>,
  input: {
  ...
  const mappings = [
    ["MERCURY_WORKSPACE_CWD", input.workspaceCwd],
    ["MERCURY_WORKSPACE_SOURCE", input.workspaceSource],
    ["MERCURY_WORKSPACE_STRATEGY", input.workspaceStrategy],
    ["MERCURY_WORKSPACE_ID", input.workspaceId],
    ["MERCURY_WORKSPACE_REPO_URL", input.workspaceRepoUrl],
    ["MERCURY_WORKSPACE_REPO_REF", input.workspaceRepoRef],
    ["MERCURY_WORKSPACE_BRANCH", input.workspaceBranch],
    ["MERCURY_WORKSPACE_WORKTREE_PATH", input.workspaceWorktreePath],
...
export function sanitizeInheritedMercuryEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...baseEnv };
  for (const key of Object.keys(env)) {
    if (!key.startsWith("MERCURY_")) continue;
    if (key === "MERCURY_RUNTIME_API_URL") continue;
    if (key === "MERCURY_LISTEN_HOST") continue;
    if (key === "MERCURY_LISTEN_PORT") continue;
    delete env[key];
  }
  return env;
}
...
export async function resolveMercurySkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...MERCURY_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listMercurySkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<MercurySkillEntry[]> {
  const root = await resolveMercurySkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      return {
        key: `StreamDemon/mercury/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Mercury skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Mercury cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredMercuryRuntimeSkills(value: unknown): MercurySkillEntry[] {
...
export async function readMercuryRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<MercurySkillEntry[]> {
  const configuredEntries = normalizeConfiguredMercuryRuntimeSkills(config.mercuryRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listMercurySkillEntries(moduleDir, additionalCandidates);
}

export async function readMercurySkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readMercurySkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.mercurySkillSync;
...
function canonicalizeDesiredMercurySkillReference(
  reference: string,
...
export function resolveMercuryDesiredSkillNames(
  config: Record<string, unknown>,
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredMercurySkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeMercurySkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.mercurySkillSync;
  const current =
  ...
  next.mercurySkillSync = current;
  return next;
}

export async function ensureMercurySkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const rawMerged: NodeJS.ProcessEnv = {
      ...sanitizeInheritedMercuryEnv(process.env),
      ...opts.env,
    };

    // Strip Claude Code nesting-guard env vars so spawned `claude` processes
    // don't refuse to start with "cannot be launched inside another session".
    // These vars leak in when the Mercury server itself is started from
    // within a Claude Code session (e.g. `npx mercury run` in a terminal
