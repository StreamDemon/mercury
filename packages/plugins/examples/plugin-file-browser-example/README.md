...
Example Mercury plugin that demonstrates:
...
```bash
pnpm --filter @mercuryai/plugin-file-browser-example build
pnpm mercuryai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm mercuryai plugin uninstall mercury-file-browser-example --force
```
...
- Optional: use `mercury-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
