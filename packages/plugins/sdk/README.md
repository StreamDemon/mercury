# `@mercuryai/plugin-sdk`

Official TypeScript SDK for Mercury plugin authors.

- **Worker SDK:** `@mercuryai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@mercuryai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@mercuryai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@mercuryai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@mercuryai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@mercuryai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@mercuryai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@mercuryai/plugin-sdk/ui/hooks` | Hooks only |
| `@mercuryai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@mercuryai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@mercuryai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@mercuryai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@mercuryai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@mercuryai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Mercury app. They can call ordinary Mercury HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Mercury instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@mercuryai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@mercuryai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Mercury information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@mercuryai/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:mercury.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@mercuryai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@mercuryai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@mercuryai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@mercuryai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@mercuryai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@mercuryai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@mercuryai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@mercuryai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@mercuryai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@mercuryai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@mercuryai/plugin-sdk/bundlers";
...
```bash
mercury-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@mercuryai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__mercury__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__mercury__/events` streams `reload` SSE events on UI build changes
