/**
 * Core types for the Mercury plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@mercuryai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @mercuryai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Mercury object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Mercury UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Mercury domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Mercury domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Mercury secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Mercury secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@mercuryai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@mercuryai/plugin-sdk";
 *
