...
import type { MercuryPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@mercuryai/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: MercuryPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@mercuryai/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
