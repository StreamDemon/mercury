 ...
 * channel.
 *
 * The Mercury plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@mercuryai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Mercury messages.
 */
...
/**
 * Mercury plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Mercury instance. */
    instanceId: string;
    /** Semver version of the running Mercury host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
