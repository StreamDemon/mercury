...
describe("E2B sandbox provider plugin", () => {
  ...
  it("uses resolved config keys before falling back to E2B_API_KEY", async () => {
    ...
    expect(lease).toMatchObject({
      ...
      metadata: {
        provider: "e2b",
        remoteCwd: "/home/user/mercury-workspace",
      },
    });
    expect(sandbox.commands.run).toHaveBeenNthCalledWith(1, "pwd");
    expect(sandbox.commands.run).toHaveBeenNthCalledWith(2, "mkdir -p '/home/user/mercury-workspace'");
  });
  ...
  it("creates the remote workspace before returning it", async () => {
    ...
    await expect(plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: { remoteCwd: "/home/user/mercury-workspace" },
      },
      workspace: {
        localPath: "/tmp/mercury-workspace",
      },
    })).resolves.toEqual({
      cwd: "/home/user/mercury-workspace",
      metadata: {
        provider: "e2b",
        remoteCwd: "/home/user/mercury-workspace",
      },
    ...
    expect(sandbox.commands.run).toHaveBeenCalledWith("mkdir -p '/home/user/mercury-workspace'");
  });
