...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".mercury-sdk");
...
/**
 * Generate a complete Mercury plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Mercury plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "mercury-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["mercury", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@mercuryai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@mercuryai/plugin-sdk": sdkDependency,
...
`,
  ...
  if (template === "environment") {
    writeFile(
      ...
      `import type { MercuryPluginManifestV1 } from "@mercuryai/plugin-sdk";

const manifest: MercuryPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@mercuryai/plugin-sdk/ui";
...
} from "@mercuryai/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@mercuryai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@mercuryai/plugin-sdk\` and \`@mercuryai/shared\` from a local Mercury checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.mercury-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-mercury-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <mercury-sdk-path>]");
    process.exit(1);
  }
