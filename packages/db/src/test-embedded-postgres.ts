...
const DEFAULT_MERCURY_EMBEDDED_POSTGRES_PORT = 54329;

function getReservedTestPorts(): Set<number> {
  const configuredPorts = [
    DEFAULT_MERCURY_EMBEDDED_POSTGRES_PORT,
    Number.parseInt(process.env.MERCURY_EMBEDDED_POSTGRES_PORT ?? "", 10),
    ...String(process.env.MERCURY_TEST_POSTGRES_RESERVED_PORTS ?? "")
      .split(",")
...
async function getAvailablePort(): Promise<number> {
  ...
  throw new Error(
    `Failed to allocate embedded Postgres test port outside reserved Mercury ports: ${[
      ...reservedPorts,
    ].join(", ")}`,
  );
}
...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  const { dataDir, instance } = await createEmbeddedPostgresTestInstance(
    "mercury-embedded-postgres-probe-",
  );
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://mercury:mercury@127.0.0.1:${port}/mercury`;
