...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://mercury:mercury@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "mercury");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://mercury:mercury@127.0.0.1:${preferredPort}/mercury`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://mercury:mercury@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "mercury");
    return {
      connectionString: `postgres://mercury:mercury@127.0.0.1:${port}/mercury`,
  ...
  const adminConnectionString = `postgres://mercury:mercury@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "mercury");

  return {
    connectionString: `postgres://mercury:mercury@127.0.0.1:${selectedPort}/mercury`,
