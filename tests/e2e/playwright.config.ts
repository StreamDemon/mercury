...
const PORT = Number(process.env.MERCURY_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const MERCURY_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp MERCURY_HOME.
  webServer: {
    command: `pnpm mercuryai onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Mercury home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      MERCURY_HOME,
      MERCURY_INSTANCE_ID: "playwright-e2e",
      MERCURY_BIND: "loopback",
      MERCURY_DEPLOYMENT_MODE: "local_trusted",
