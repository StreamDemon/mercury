...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Mercury.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.mercury.yaml`
...
- Mercury must not require extra top-level fields for skill validity
- Mercury-specific extensions must live under `metadata.mercury` or `metadata.sources`
...
For Mercury, the preferred fidelity extension is:
...
- issue/task Mercury-only metadata
...
- Mercury tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Mercury shape:
...
Additional rules for Mercury exporters:
...
- omit empty and default-valued Mercury fields when possible
...
- Mercury exporters should emit `.mercury.yaml` as a sidecar by default
...
Mercury can map this spec to its runtime model like this:
...
- Mercury extension:
  - `.mercury.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Mercury-specific fidelity

Inline Mercury-only metadata that must live inside a shared markdown file should use:

- `metadata.mercury`

That keeps the base format broader than Mercury.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Mercury.

## 20. Cutover

Mercury should cut over to this markdown-first package model as the primary portability format.

`mercury.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Mercury, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
