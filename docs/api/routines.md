...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Mercury-Signature` + `X-Mercury-Timestamp` header pair matching the trigger's signing mode.
