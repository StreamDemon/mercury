# ClipHub: Marketplace for Mercury Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Mercury configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Mercury-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Mercury company (agents, projects, governance) |
...
A complete Mercury company configuration:
...
Single-agent configurations designed to plug into a Mercury org:
...
Portable skill files that any Mercury agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Mercury's skill loading system
...
interface Listing {
  ...
  mercuryVersionMin: string;     // Minimum Mercury version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Mercury company
...
| `POST` | `/api/listings/:id/install` | Install to Mercury company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Mercury company (or create new)
  → ClipHub API calls Mercury API to:
  ...
  → Redirect to Mercury dashboard with new team running
```
...
### 5.3 Creator: Export from Mercury → Publish

```
Running Mercury company → "Export as Blueprint" (CLI or UI)
  → Mercury exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Mercury brand blue/purple)
...
| Install flow | One-click deploy to Mercury company |
...
{
  "targetCompanyId": "uuid",         // Existing Mercury company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `mercury-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Mercury
- **Backend**: Node.js API (or extend Mercury server)
- **Database**: Postgres (can share Mercury's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Mercury auth (or OAuth2)

### 9.2 Integration with Mercury

ClipHub can be:
- **Option A**: A separate app that calls Mercury's API to install blueprints
- **Option B**: A built-in section of the Mercury UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Mercury UI and API.
...
- [ ] Install flow: blueprint → Mercury company
...
- [ ] "Export from Mercury" CLI command
