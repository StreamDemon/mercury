...
Mercury companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .mercury.yaml     # Adapter config, env inputs, routines
```
...
- **.mercury.yaml** holds Mercury-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
mercuryai company export <company-id> --out ./my-export
```
...
mercuryai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
mercuryai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
mercuryai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.mercury.yaml`
...
```sh
# From a local folder
mercuryai company import ./my-export

# From a GitHub URL
mercuryai company import https://github.com/org/repo

# From a GitHub subfolder
mercuryai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
mercuryai company import org/repo
mercuryai company import org/repo/companies/acme
```
...
If `--target` is not specified, Mercury infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
mercuryai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
mercuryai company import org/company-templates/engineering-team \
  --target new \
...
```sh
mercuryai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
mercuryai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
mercuryai company import ./package \
  --target new \
  --yes \
  --json
```
...
Mercury supports several GitHub URL formats:
