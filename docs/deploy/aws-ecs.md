...
summary: Deploy Mercury to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Mercury to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The Mercury repo cloned locally
...
export MERCURY_DOMAIN=mercury.example.com   # your domain
...
aws ecr create-repository \
  --repository-name mercury-server \
...
# Tag and push
docker tag mercury-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/mercury-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name mercury-alb \
  --description "Mercury ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name mercury-ecs \
  --description "Mercury ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name mercury-rds \
  --description "Mercury RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name mercury-efs \
  --description "Mercury EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name mercury-db-subnet \
  --db-subnet-group-description "Mercury RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier mercury-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username mercury \
...
DATABASE_URL="postgresql://mercury:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/mercury"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=mercury-data \
...
aws secretsmanager create-secret \
  --name mercury/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name mercury/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name mercury/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name mercury/openai-api-key \
...
# Task execution role
aws iam create-role \
  --role-name mercury-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:mercury/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name mercury-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name mercury

aws logs create-log-group --log-group-name /ecs/mercury
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$MERCURY_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/mercury-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/mercury-task-def.json
```
...
- Create a CNAME or ALIAS record for `$MERCURY_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster mercury \
  --service-name mercury-server \
  --task-definition mercury-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs describe-services \
  --cluster mercury \
  --services mercury-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster mercury --service-name mercury-server
TASK_ARN=$(aws ecs list-tasks --cluster mercury --service-name mercury-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster mercury --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/mercury --since 10m --follow

# Hit the health endpoint
curl -sf https://$MERCURY_DOMAIN/api/health
```
...
#   { "name": "MERCURY_AUTH_DISABLE_SIGN_UP", "value": "true" }
...
aws ecs update-service \
  --cluster mercury \
  --service mercury-server \
  --task-definition mercury-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier mercury-db
aws rds start-db-instance --db-instance-identifier mercury-db
```
...
aws ecs update-service --cluster mercury --service mercury-server --desired-count 0
aws ecs delete-service --cluster mercury --service mercury-server --force
aws ecs delete-cluster --cluster mercury
...
aws rds delete-db-instance \
  --db-instance-identifier mercury-db \
  --final-db-snapshot-identifier mercury-db-final
aws rds wait db-instance-deleted --db-instance-identifier mercury-db
aws rds delete-db-subnet-group --db-subnet-group-name mercury-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id mercury/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name mercury-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name mercury-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name mercury-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name mercury-ecs-execution
aws iam delete-role --role-name mercury-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/mercury
```
