...
Mercury supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm mercuryai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Mercury emits a one-time claim URL at startup:
...
```sh
MERCURY_DEPLOYMENT_MODE=authenticated MERCURY_BIND=lan pnpm mercuryai run
```
