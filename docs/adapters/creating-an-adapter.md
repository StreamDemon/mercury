...
Build a custom adapter to connect Mercury to any agent runtime.
...
|---|---|---|
| Source | Inside `mercury-fork` | Separate npm package |
| Distribution | Ships with Mercury | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Mercury's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@mercuryai/adapter-utils/server-utils`
2. Build environment with `buildMercuryEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@mercuryai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildMercuryEnv(agent)` | `@mercuryai/adapter-utils/server-utils` | Inject `MERCURY_*` env vars |
| `renderTemplate(tpl, data)` | `@mercuryai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@mercuryai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@mercuryai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Mercury's source:
...
`format-event.ts` — pretty-prints stdout for `mercuryai run --watch` using `picocolors`.
...
With these flags set, the Mercury UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Mercury source changes required.
...
Make Mercury skills discoverable to your agent runtime without writing to the agent's working directory:
