...
summary: Build, package, and distribute adapters as plugins without modifying Mercury source
---

Mercury supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Mercury's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `mercury-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Mercury | Published to npm or linked via `file:` |
| Updates | Requires Mercury release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@mercuryai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `mercury.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
export async function execute(
  ...
  // 2. Build environment with Mercury vars injected
  const env = buildMercuryEnv(agent);
...
#### Available Helpers from `@mercuryai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildMercuryEnv(agent)` | Inject `MERCURY_*` environment variables |
...
```sh
# Via the Mercury UI
# Settings → Adapters → Install from npm → "my-mercury-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-mercury-adapter"}'
```
...
Local adapters are symlinked into Mercury's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.mercury/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-mercury-adapter",
...
Other Mercury users can then install your adapter by package name from the UI or API.
