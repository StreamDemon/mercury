...
- Running a Python script that calls the Mercury API
...
1. Mercury spawns the configured command as a child process
2. Standard Mercury environment variables are injected (`MERCURY_AGENT_ID`, `MERCURY_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Mercury API and perform work.
