...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Mercury saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
pnpm mercuryai feedback report
```
...
```bash
# Installed CLI
mercuryai feedback report

# Point to a different server or company
pnpm mercuryai feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm mercuryai feedback report --payloads
```
...
```bash
pnpm mercuryai feedback export
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Mercury feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Mercury envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm mercuryai feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, Mercury falls back to `https://telemetry.mercury.ing`
