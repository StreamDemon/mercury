...
Add first-class **documents** to Mercury as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Mercury workflow/docs.
...
Update the Mercury agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/mercury/SKILL.md`
