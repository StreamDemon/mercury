/**
 * esbuild configuration for building the mercuryai CLI for npm.
 *
 * Bundles all workspace packages (@mercuryai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @mercuryai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@mercuryai/")) {
      externals.add(name);
    }
  }
