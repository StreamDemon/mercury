...
export const DEFAULT_WORKTREE_HOME = "~/.mercury-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".mercury");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: MercuryConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "mercury",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    MERCURY_HOME: paths.homeDir,
    MERCURY_INSTANCE_ID: paths.instanceId,
    MERCURY_CONFIG: paths.configPath,
    MERCURY_CONTEXT: paths.contextPath,
    MERCURY_IN_WORKTREE: "true",
    ...(branding?.name ? { MERCURY_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { MERCURY_WORKTREE_COLOR: branding.color } : {}),
  };
}
