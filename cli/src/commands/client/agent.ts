...
import type { Agent } from "@mercuryai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveMercurySkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export MERCURY_API_URL='${escaped(input.apiBase)}'`,
    `export MERCURY_COMPANY_ID='${escaped(input.companyId)}'`,
    `export MERCURY_AGENT_ID='${escaped(input.agentId)}'`,
    `export MERCURY_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Mercury skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Mercury skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveMercurySkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Mercury skills directory. Expected ./skills in the repo checkout.",
              );
            }
