...
import { ApiRequestError, MercuryApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Mercury config file")
    .option("-d, --data-dir <path>", "Mercury data directory root (isolates state from ~/.mercury)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Mercury API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.MERCURY_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.MERCURY_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.MERCURY_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set MERCURY_COMPANY_ID, or set context profile companyId via `mercuryai context set`.",
    );
  }

  const api = new MercuryApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.MERCURY_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.MERCURY_SERVER_PORT || "");
