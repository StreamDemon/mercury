...
type OnboardDefaults = Pick<MercuryConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or MERCURY_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "MERCURY_PUBLIC_URL",
  "DATABASE_URL",
  "MERCURY_DB_BACKUP_ENABLED",
  "MERCURY_DB_BACKUP_INTERVAL_MINUTES",
  "MERCURY_DB_BACKUP_RETENTION_DAYS",
  "MERCURY_DB_BACKUP_DIR",
  "MERCURY_DEPLOYMENT_MODE",
  "MERCURY_DEPLOYMENT_EXPOSURE",
  "MERCURY_BIND",
  "MERCURY_BIND_HOST",
  "MERCURY_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "MERCURY_ALLOWED_HOSTNAMES",
  "MERCURY_AUTH_BASE_URL_MODE",
  "MERCURY_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "MERCURY_STORAGE_PROVIDER",
  "MERCURY_STORAGE_LOCAL_DIR",
  "MERCURY_STORAGE_S3_BUCKET",
  "MERCURY_STORAGE_S3_REGION",
  "MERCURY_STORAGE_S3_ENDPOINT",
  "MERCURY_STORAGE_S3_PREFIX",
  "MERCURY_STORAGE_S3_FORCE_PATH_STYLE",
  "MERCURY_SECRETS_PROVIDER",
  "MERCURY_SECRETS_STRICT_MODE",
  "MERCURY_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<MercuryConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveMercuryInstanceId();
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.MERCURY_PUBLIC_URL?.trim() ||
      process.env.MERCURY_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.MERCURY_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.MERCURY_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.MERCURY_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.MERCURY_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.MERCURY_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.MERCURY_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.MERCURY_ALLOWED_HOSTNAMES
    ? process.env.MERCURY_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.MERCURY_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.MERCURY_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.MERCURY_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.MERCURY_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.MERCURY_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.MERCURY_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.MERCURY_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.MERCURY_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.MERCURY_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.MERCURY_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.MERCURY_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.MERCURY_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.MERCURY_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.MERCURY_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.MERCURY_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "MERCURY_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.MERCURY_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "MERCURY_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.MERCURY_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "MERCURY_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<MercuryConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  let existingConfig: MercuryConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Mercury install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("mercuryai configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("MERCURY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.MERCURY_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("MERCURY_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("MERCURY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: MERCURY_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("mercuryai run")}`,
        `Reconfigure later: ${pc.cyan("mercuryai configure")}`,
        `Diagnose setup: ${pc.cyan("mercuryai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.MERCURY_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Mercury setup is ready.");
    return;
  }
  ...
  let llm: MercuryConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@mercuryai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `mercuryai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("mercuryai run")}`,
        `Then: ${pc.cyan("mercuryai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
