...
const DEFAULT_AGENT_JWT_ISSUER = "mercury";
const DEFAULT_AGENT_JWT_AUDIENCE = "mercury-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveMercuryInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveMercuryInstanceId());
}
...
function collectDeploymentEnvRows(config: MercuryConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrlSource: EnvSource =
    process.env.MERCURY_PUBLIC_URL
      ? "env"
      : process.env.MERCURY_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.MERCURY_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.MERCURY_SECRETS_STRICT_MODE ??
    String(config?.secrets?.strictMode ?? false);
  const secretsKeyFilePath =
    process.env.MERCURY_SECRETS_MASTER_KEY_FILE ??
    config?.secrets?.localEncrypted?.keyFilePath ??
    defaultSecretsKeyFilePath();
  const storageProvider =
    process.env.MERCURY_STORAGE_PROVIDER ??
    config?.storage?.provider ??
    DEFAULT_STORAGE_PROVIDER;
  const storageLocalDir =
    process.env.MERCURY_STORAGE_LOCAL_DIR ??
    config?.storage?.localDisk?.baseDir ??
    defaultStorageBaseDir();
  const storageS3Bucket =
    process.env.MERCURY_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "mercury";
  const storageS3Region =
    process.env.MERCURY_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.MERCURY_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.MERCURY_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.MERCURY_STORAGE_S3_FORCE_PATH_STYLE ??
    String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "MERCURY_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from MERCURY_PUBLIC_URL when possible)",
    },
    {
      key: "MERCURY_AGENT_JWT_TTL_SECONDS",
      value: process.env.MERCURY_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.MERCURY_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "MERCURY_AGENT_JWT_ISSUER",
      value: process.env.MERCURY_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.MERCURY_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "MERCURY_AGENT_JWT_AUDIENCE",
      value: process.env.MERCURY_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.MERCURY_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "MERCURY_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.MERCURY_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.MERCURY_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "MERCURY_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.MERCURY_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.MERCURY_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.MERCURY_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.MERCURY_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.MERCURY_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.MERCURY_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.MERCURY_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "MERCURY_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.MERCURY_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.MERCURY_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "MERCURY_CONFIG",
      value: process.env.MERCURY_CONFIG ?? configPath,
      source: process.env.MERCURY_CONFIG ? "env" : "default",
      required: false,
