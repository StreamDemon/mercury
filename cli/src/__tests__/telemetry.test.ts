...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.MERCURY_HOME = path.join(root, "home");
    process.env.MERCURY_INSTANCE_ID = "telemetry-test";
