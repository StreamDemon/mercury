...
function createBaseMercuryEnv(options: TestMercuryEnv) {
  ...
  env.MERCURY_CONFIG = options.configPath;
  env.MERCURY_HOME = options.mercuryHome;
  env.MERCURY_INSTANCE_ID = options.instanceId;
  env.MERCURY_CONTEXT = path.join(options.mercuryHome, "context.json");
  env.MERCURY_AUTH_STORE = path.join(options.mercuryHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestMercuryEnv, "configPath">,
) {
  const env = createBaseMercuryEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.MERCURY_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.MERCURY_MIGRATION_AUTO_APPLY = "true";
  env.MERCURY_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestMercuryEnv) {
  const env = createBaseMercuryEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.MERCURY_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.MERCURY_MIGRATION_AUTO_APPLY;
  delete env.MERCURY_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(
  args: string[],
  opts: TestMercuryEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "mercuryai", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `mercuryai run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("mercuryai company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  let mercuryHome = "";
  let cliShellHome = "";
  let mercuryInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "mercury-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    mercuryHome = path.join(tempRoot, "mercury-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    mercuryInstanceId = "company-cli-e2e";
    mkdirSync(mercuryHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("mercury-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["mercuryai", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          mercuryHome,
          instanceId: mercuryInstanceId,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(mercuryHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".mercury", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".mercury.yaml"), "utf8")).toContain('schema: "mercury/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "mercury-demo"));
