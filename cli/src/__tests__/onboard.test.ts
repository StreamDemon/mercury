...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".mercury", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-onboard-fresh-"));
  return path.join(root, ".mercury", "config.json");
}
...
describe("onboard", () => {
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.MERCURY_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as MercuryConfig;
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.MERCURY_TAILNET_BIND_HOST = "100.64.0.8";
  ...
  it("keeps tailnet quickstart on loopback until tailscale is available", async () => {
    const configPath = createFreshConfigPath();
    delete process.env.MERCURY_TAILNET_BIND_HOST;
  ...
  it("ignores deployment env overrides during --yes quickstart", async () => {
    const configPath = createFreshConfigPath();
    process.env.MERCURY_DEPLOYMENT_MODE = "authenticated";
