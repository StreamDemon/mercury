...
import { ApiConnectionError, ApiRequestError, MercuryApiClient } from "../client/http.js";

describe("MercuryApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new MercuryApiClient({
    ...
    expect(headers["x-mercury-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new MercuryApiClient({ apiBase: "http://localhost:3100" });
