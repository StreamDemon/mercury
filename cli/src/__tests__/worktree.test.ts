...
function buildSourceConfig(): MercuryConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("mercury-pr-432")).toBe(
      path.resolve(os.homedir(), "mercury-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("mercury/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://mercury.example", 3110)).toBe("https://mercury.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/mercury-feature",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/mercury-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/mercury-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.MERCURY_HOME).toBe(path.resolve("/tmp/mercury-worktrees"));
    expect(env.MERCURY_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.MERCURY_IN_WORKTREE).toBe("true");
    expect(env.MERCURY_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.MERCURY_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export MERCURY_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("mercury-worktree-quarantine-");
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-secrets-"));
    const originalInlineMasterKey = process.env.MERCURY_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.MERCURY_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.MERCURY_SECRETS_MASTER_KEY;
      delete process.env.MERCURY_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.MERCURY_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.MERCURY_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.MERCURY_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          MERCURY_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.MERCURY_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.MERCURY_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".mercury-worktrees"),
      });

      const envPath = path.join(repoRoot, ".mercury", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("MERCURY_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("MERCURY_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/MERCURY_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.MERCURY_AGENT_JWT_SECRET;
      } else {
        process.env.MERCURY_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      ...
      const worktreeHome = path.join(tempRoot, ".mercury-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("mercury-worktree-auth-source-");

      try {
        const sourceDbClient = createDb(sourceDb.connectionString);
        await sourceDbClient.insert(authUsers).values({
          id: "user-existing",
          email: "existing@modernmethod.io",
          name: "Existing User",
          emailVerified: true,
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        const targetConfig = JSON.parse(
          fs.readFileSync(path.join(worktreeRoot, ".mercury", "config.json"), "utf8"),
        ) as MercuryConfig;
        ...
        try {
          const targetDb = createDb(
            `postgres://mercury:mercury@127.0.0.1:${targetConfig.database.embeddedPostgresPort}/mercury`,
          );
          const seededUsers = await targetDb.select().from(authUsers);
          expect(seededUsers.some((row) => row.email === "existing@modernmethod.io")).toBe(true);
        } finally {
          await targetPg.stop();
        }
      } finally {
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".mercury-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".mercury", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Mercury config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".mercury", "config.json");
    const originalCwd = process.cwd();
    const originalMercuryConfig = process.env.MERCURY_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalMercuryConfig === undefined) {
        delete process.env.MERCURY_CONFIG;
      } else {
        process.env.MERCURY_CONFIG = originalMercuryConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".mercury", "config.json");
    const envPath = path.join(worktreeRoot, ".mercury", ".env");
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".mercury", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".mercury", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Mercury instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".mercury-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalMercuryConfig = process.env.MERCURY_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `MERCURY_HOME=${homeDir}`,
          `MERCURY_INSTANCE_ID=${currentInstanceId}`,
          "MERCURY_WORKTREE_NAME=existing-name",
          "MERCURY_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`MERCURY_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("MERCURY_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("MERCURY_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalMercuryConfig === undefined) {
        delete process.env.MERCURY_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".mercury-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `MERCURY_HOME=${homeDir}\nMERCURY_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`MERCURY_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/mercury",
        targetRepoRoot: "/Users/example/mercury-pr-432",
        workspaceCwd: "/Users/example/mercury",
      }),
    ).toBe("/Users/example/mercury-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/mercury",
        targetRepoRoot: "/Users/example/mercury-pr-432",
        workspaceCwd: "/Users/example/mercury/packages/db",
      }),
    ).toBe("/Users/example/mercury-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "mercury-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("mercury-make-test", {
        seed: false,
        home: path.join(tempRoot, ".mercury-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".mercury", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".mercury", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".mercury", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".mercury", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Mercury metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".mercury", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".mercury-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mercury-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".mercury-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".mercury", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".mercury", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".mercury", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("mercury-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();
