import { readConfig, configExists, resolveMercuryConfigPath } from "../config/store.js";
...
export function configCheck(configPath?: string): CheckResult {
  ...
  if (!configExists(configPath)) {
    return {
      ...
      canRepair: false,
      repairHint: "Run `mercuryai onboard` to create one",
    };
  }
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Run `mercuryai configure --section database` (or `mercuryai onboard` to recreate)",
    };
  }
}
