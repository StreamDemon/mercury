...
export class MercuryApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-mercury-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Mercury API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Mercury server is not running, the configured URL is wrong, or the request is being blocked before it reaches Mercury.",
    "",
    "Try:",
    "- Start Mercury with `pnpm dev` or `pnpm mercuryai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Mercury is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`MERCURY_API_URL\`.`,
  );
  return lines.join("\n");
}
