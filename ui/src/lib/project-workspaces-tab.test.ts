...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, WorkspaceRuntimeService } from "@mercuryai/shared";
...
function createExecutionWorkspace(overrides: Partial<ExecutionWorkspace>): ExecutionWorkspace {
  return {
    ...
    name: overrides.name ?? "MERC-893",
    status: overrides.status ?? "active",
    cwd: overrides.cwd ?? "/repo/.worktrees/MERC-893",
    repoUrl: overrides.repoUrl ?? null,
    baseRef: overrides.baseRef ?? "public-gh/master",
    branchName: overrides.branchName ?? "MERC-893-workspaces-tab",
    ...
    const summaries = buildProjectWorkspaceSummaries({
      ...
      executionWorkspaces: [
        createExecutionWorkspace({
          id: "exec-1",
          name: "MERC-893",
          branchName: "MERC-893-workspaces-tab",
