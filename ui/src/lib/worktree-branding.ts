...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("mercury-worktree-enabled") !== "true") return null;

  const name = readMetaContent("mercury-worktree-name");
  const color = normalizeHexColor(readMetaContent("mercury-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("mercury-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
