...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="mercury-markdown-issue-ref"
      title={title}
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`mercury-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="mercury-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("mercury-mermaid-status", error && "mercury-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="mercury-mermaid-source">
...
export function MarkdownBody({
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `mercury-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "mercury-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "mercury-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
