...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@mercuryai/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@mercuryai/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@mercuryai/adapter-utils").TranscriptEntry[];
