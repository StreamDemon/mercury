...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Mercury API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "mercuryApiUrl",
                  String(config.mercuryApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "mercuryApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://mercury.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/mercury-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "mercury"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Mercury persists a device key during onboarding so pairing approvals
