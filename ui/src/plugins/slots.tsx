 ...
 *
 * Plugin bundles are built with `external: ["@mercuryai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__mercuryPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__mercuryPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__mercuryPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__mercuryPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@mercuryai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@mercuryai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@mercuryai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@mercuryai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@mercuryai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__mercuryPluginBridge__) {
 ...
 * self-register via `window.mercuryPlugins.registerReactComponent()`.
 ...
 * of `@mercuryai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
