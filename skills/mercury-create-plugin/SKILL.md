---
name: mercury-create-plugin
description: >
  Create new Mercury plugins with the current alpha SDK/runtime. Use when
...
# Create a Mercury Plugin

Use this skill when the task is to create, scaffold, or document a Mercury plugin.
...
node packages/plugins/create-mercury-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Mercury repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.mercury-sdk/`:

```bash
pnpm --filter @mercuryai/create-mercury-plugin build
node packages/plugins/create-mercury-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Mercury from an absolute local path during development
